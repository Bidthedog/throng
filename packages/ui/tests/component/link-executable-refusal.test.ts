...
beforeEach(() => {
  ...
  (window as unknown as { throng: unknown }).throng = {
    links: {
      ...
      // in-project file and reveals everything else itself — never opens it (FR-111).
...
async function terminalCtrlClick(resolved: ResolvedLink, text: string, s: Surface): Promise<void> {
  ...
  const provider = createFileLinkProvider({
    ...
    follow: ({ request }: { request: LinkResolutionRequest }) => {
      followed = followLink({ request, deps: s.deps });
    },
...
async function editorChord(resolved: ResolvedLink, text: string, s: Surface): Promise<void> {
  ...
  const deps = {
    ...
    ask: (request: LinkResolutionRequest) => (request.text === text ? answer(resolved)() : { ok: false }),
    follow: (hit: EditorLinkAt) => {
      if (hit.kind !== 'file') return;
      followed = followLink({ request: hit.request, deps: s.deps });
    },
  } as EditorLinkDeps;
  const view = {
...
describe('FR-111 — the explicit menu item still runs it', () => {
  it('every reported extension runs from *Open Program*, never *Open in OS Default Program* (S8)', async () => {
    ...
    for (const extension of EXECUTABLES) {
      ...
      const runDefaultProgram = () =>
        void performLinkTarget({ target: 'osDefaultProgram', link: resolved, request, deps: { ...s.deps, ...osLinkActions() } });
      const actions = linkMenuActions(
        {
          cls: 'onDevice',
          applicable: {
          ...
          performers: {
            ...
            openInPreview: () => {},
            osExplorer: () =>
              void performLinkTarget({ target: 'osExplorer', link: resolved, request, deps: { ...s.deps, ...osLinkActions() } }),
            osDefaultProgram: runDefaultProgram,
            // S8 — the SAME route: opening an executable with its default handler runs it.
            openProgram: runDefaultProgram,
          },
        },
        { kind: resolved.kind, inProject: resolved.inProject, executable: resolved.executable, preview: resolved.preview },
      );
      expect(
        actions.find((a) => a.label === 'Open in OS Default Program'),
        `${extension}: offered Open in OS Default Program for an executable`,
      ).toBeUndefined();
      const row = actions.find((a) => a.label === 'Open Program');
      if (!row) {
        refused.push(`${extension}: the menu offered no Open Program row`);
        continue;
      }
      await row.onClick?.();
