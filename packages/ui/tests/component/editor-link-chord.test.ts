...
describe('G8 — a single caret inside a link follows it (FR-044)', () => {
  it('follows, and says it claimed the key', () => {
    ...
    // T263: the hit carries the request main will resolve at the follow, not an answer (§16.17).
...
describe('G9 — anywhere else the chord keeps its editor meaning (FR-044)', () => {
  ...
  /*
   * *Round four (T263; FR-155, FR-160):* this case asserted that a caret in a path that does not
   * resolve claimed nothing (FR-006). A path is a link by grammar now, so the chord claims it and the
   * follow asks main — which answers `notFound` and raises the one notice. Prose the grammar refuses
   * still claims nothing, which is what keeps Ctrl+Enter's blank line everywhere else (G9).
   */
  it('a caret inside a path with nothing behind it CLAIMS it — main decides at the follow (FR-160)', () => {
    const doc = 'see nope/missing.ts here';
    const d = deps();
    const state = EditorState.create({ doc, selection: EditorSelection.cursor(6) });
    expect(followLinkAtCaret({ state, visibleRanges: [] }, d)).toBe(true);
    expect(d.followed.map((h) => h.request.text)).toEqual(['nope/missing.ts']);
  });

  it('a caret inside prose the grammar refuses claims nothing', () => {
    const doc = 'e.g. version 1.2.3 is fine';
    const d = deps();
    const state = EditorState.create({ doc, selection: EditorSelection.cursor(2) });
