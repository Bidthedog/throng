...
describe('ElectronShellIntegration — what openWithDefaultProgram does once it accepts (FR-036)', () => {
  ...
  it('SI3: each refusal names the path AND says which of the two it was', async () => {
    const h = makeHarness();
    const gone = await h.shell
      .openWithDefaultProgram(h.fixtures!.missing)
      .then(() => '')
      .catch((e: Error) => e.message);
    const folder = await h.shell
      .openWithDefaultProgram(h.fixtures!.existingFolder)
      .then(() => '')
      .catch((e: Error) => e.message);
  ...
  it('reports the OS’s own failure text when the handler refuses the file', async () => {
    let calls = 0;
    ...
    const message = await subject
      .openWithDefaultProgram(fixtures.existingFile)
      .then(() => '')
      .catch((e: Error) => e.message);
    expect(calls).toBe(1);
    expect(message).toContain(fixtures.existingFile);
    expect(message).toContain('No application is associated with this file');
  });
});
