...
function fakeService() {
  ...
  const service: LinkIpcService = {
    ...
    // 045 T293 — the follow (data-model §16.18): one request per Ctrl+click.
    follow: async (request: LinkResolutionRequest) => {
      seen.push({ op: 'follow', request });
...
describe('throng:links:* \u2014 the four channels exist, and all four are invoke', () => {
  it('registers exactly resolve, reveal, open and follow', () => {
    const ipc = fakeIpc();
...
describe('throng:links:* \u2014 what each handler forwards, and what it answers', () => {
  ...
  it('a non-link comes back as a VALUE, not as a rejection across the bridge', async () => {
    ...
    registerLinkIpc(ipc, {
      ...
      follow: async () => ({ kind: 'notFound', path: 'D:\\p\\a.txt' }),
    } as LinkIpcService);
    await expect(ipc.handles.get('throng:links:resolve')!(event(1), REQUEST)).resolves.toEqual({ ok: false });
    await expect(ipc.handles.get('throng:links:follow')!(event(1), REQUEST)).resolves.toEqual({
      kind: 'notFound',
      path: 'D:\\p\\a.txt',
    });
  });
  ...
  it('a service that throws answers a value too \u2014 a rejection would surface as an opaque bridge error', async () => {
    ...
    // data-model \u00a716.21 / \u00a716.22: a throwing follow answers `failed`, naming the path the user named and
    // carrying the message \u2014 `refused` stays the OS declining (FR-036).
    await expect(ipc.handles.get('throng:links:follow')!(event(1), REQUEST)).resolves.toEqual({
      kind: 'failed',
      path: REQUEST.text,
      message: 'boom',
  ...
  it('follow forwards the request and returns the outcome', async () => {
    const ipc = fakeIpc();
    const { service, seen } = fakeService();
    registerLinkIpc(ipc, service);
    await expect(ipc.handles.get('throng:links:follow')!(event(1), REQUEST)).resolves.toEqual({ kind: 'revealed' });
    expect(seen).toEqual([{ op: 'follow', request: REQUEST }]);
  });
});

/**
 * 045 T293 \u2014 `throng:links:follow` is held to the same I1/I2 whitelist as the other three
 * (data-model \u00a716.21, \u00a716.22). A request failing it answers `{ kind: 'rejected' }` \u2014 no gesture can
 * build one, so the renderer raises nothing \u2014 and reaches no service. An unknown or absent project id
 * is NOT a refusal: main derives no root and judges the target out-of-project (M3).
 */
describe('throng:links:follow \u2014 I1/I2: the whitelist and the root derivation', () => {
  for (const [label, payload] of [
    ['no panelId', { text: 'x', kind: 'detectedPath' }],
    ['an unknown kind', { ...REQUEST, kind: 'whatever' }],
    ['empty text', { ...REQUEST, text: '' }],
    ['not an object', 'src/foo.ts'],
  ] as const) {
    it(`${label} answers { kind: 'rejected' } and reaches no service`, async () => {
      const ipc = fakeIpc();
      const { service, seen } = fakeService();
      registerLinkIpc(ipc, service);
      await expect(ipc.handles.get('throng:links:follow')!(event(1), payload)).resolves.toEqual({ kind: 'rejected' });
      expect(seen).toEqual([]);
    });
  }

  it('forwards only the whitelisted fields \u2014 a smuggled root or path is dropped', async () => {
    const ipc = fakeIpc();
    const { service, seen } = fakeService();
    registerLinkIpc(ipc, service);
    await ipc.handles.get('throng:links:follow')!(event(1), {
      ...REQUEST,
  ...
  it('an unknown or absent originProjectId is forwarded as asked \u2014 main judges it out-of-project, not rejected', async () => {
    const ipc = fakeIpc();
    const { service, seen } = fakeService();
    registerLinkIpc(ipc, service);
    await expect(
      ipc.handles.get('throng:links:follow')!(event(1), { ...REQUEST, originProjectId: 'no-such-project' }),
    ).resolves.toEqual({ kind: 'revealed' });
    await expect(ipc.handles.get('throng:links:follow')!(event(1), REQUEST)).resolves.toEqual({ kind: 'revealed' });
    expect(seen.map((s) => s.request.originProjectId)).toEqual(['no-such-project', undefined]);
  });
});
...
describe('throng:links:* \u2014 preload parity for window.throng.links', () => {
  it('the preload invokes all four channels, and sends none', () => {
    const preload = source('../../src/preload/preload.cts');
    for (const channel of ['resolve', 'reveal', 'open', 'follow']) {
  ...
  it('the renderer declaration names the surface and all four members', () => {
    const globals = source('../../src/renderer/global.d.ts');
    expect(globals).toContain('links?: {');
    for (const member of ['resolve:', 'reveal:', 'open:', 'follow:']) {
      expect(globals).toContain(member);
    }
    // `window.throng.links.follow` answers the follow's outcome type, not a resolution (§16.18).
    expect(globals).toMatch(/follow:\s*\(\s*req:[^=]*\)\s*=>\s*Promise<[^>]*LinkFollowOutcome>/);
  });
 ...
 */
describe('throng:links:* — T147: reason ‘unreachable’ passes through all four channels', () => {
  function unreachableService(): LinkIpcService {
    return {
      ...
      follow: async () => ({ kind: 'unreachable', path: '\\\\fileserver\\home\\a.txt' }),
    } as LinkIpcService;
  }

  it('follow answers the unreachable outcome, path and all (T293)', async () => {
    const ipc = fakeIpc();
...
describe('throng:links:* — T205 / I7: the whitelist admits wslFlavour: true and nothing else', () => {
  ...
  it('reveal, open and follow carry it too — they resolve the same text again (I3)', async () => {
    const ipc = fakeIpc();
    const { service, seen } = fakeService();
    registerLinkIpc(ipc, service);
    ...
    await ipc.handles.get('throng:links:follow')!(event(1), { ...REQUEST, wslFlavour: true });
    expect(seen.map((s) => s.request.wslFlavour)).toEqual([true, true, true]);
  });
});
