...
| `pane-shortcuts.e2e.ts` | 2 | 2 | 2 | PARTIAL | The shared app serves the Ctrl+B / Ctrl+N test (its project sits on a path that never exists, so no watcher and no root clash; the afterEach re-expands any collapsed rail). The keybindings.json test keeps runOwnApp — the F7 rebinding is written before launch and read at it. 046 T048a removed the third test, an exact duplicate of window-chord-resolution.e2e.ts's pane-toggle test. |
...
| `window-chord-resolution.e2e.ts` | 10 | 1 | 1 | ALREADY-SHARED | One launch already serves 10 tests — at the floor. 046 T049's test adds a second project and a real cmd terminal to the shared app, and runs last so no other test inherits them. |
