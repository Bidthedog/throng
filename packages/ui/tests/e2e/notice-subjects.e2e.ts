 ...
 * theft is not the reason; the shell is.
 *
 ...
 */
test('a file failure names the file, not "this item"', async () => {
 ...
 */
test('a panel failure names Project — Tab — Panel', async () => {
  ...
  try {
    await runApp(async (_app, win) => {
      ...
      // The claim reads what has been SAVED, and the layout write is debounced.
 ...
 * live session.
 */
test('a terminal failure names its flavour', async () => {
 ...
 */
test('two different failures about one file name it identically', async () => {
 ...
 */
test('a long subject is truncated and nothing else about the notice changes', async () => {
