import type { LinkActionOutcome, LinkFollowOutcome, LinkResolution, LinkResolutionRequest } from '@throng/core';
 ...
 *
 * Four channels, all `invoke`, all answering the window that asked: `follow` for a Ctrl+click, the
 * chord and Open Link (one bounded pass, data-model §16.18); `resolve` for the Link menu's one resolution
 * when it opens; `reveal` and `open` for the menu's explicit OS items. There is no `send` and no
...
export interface LinkIpcService {
  ...
  follow(request: LinkResolutionRequest): Promise<LinkFollowOutcome>;
}
...
export function registerLinkIpc(ipcMain: LinkIpcMain, service: LinkIpcService): void {
  ...
  /*
   * 045 T294 (data-model §16.21, §16.22). A request failing the whitelist is `rejected` — no user gesture
   * can build one, so the renderer raises nothing. A service that throws is `failed`, naming the text the
   * user followed and carrying the message; `refused` stays reserved for the OS declining (FR-036).
   */
  ipcMain.handle('throng:links:follow', async (_event, payload): Promise<LinkFollowOutcome> => {
    const request = sanitise(payload);
    if (request === null) return { kind: 'rejected' };
    try {
      return await service.follow(request);
    } catch (error) {
