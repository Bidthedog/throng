...
import { registerLinkUriIpc, registerOpenExternalIpc } from './external-url.js';
...
if (isPrimaryInstance)
  ...
  // 045 round four (FR-157 – FR-159): a terminal's and an editor's web, loopback and allowlisted
  // protocol links leave by their own channel, behind the link policy — the allowlist read per request
  // from the live settings, the refused set core's half united with the platform's.
  registerLinkUriIpc(
    ipcMain,
    container.get<IRefusedUriSchemes>(UI_TYPES.RefusedUriSchemes),
    () => currentSettings.editor.links.protocolAllowlist,
    shellIntegration,
    container.get<IForegroundHandoff>(UI_TYPES.ForegroundHandoff),
    (line) => diagnostics.log.info(line),
  );
