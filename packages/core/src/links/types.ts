...
export type LinkResolution =
  ...
  | {
      ...
      /**
       * 045 FR-170a (data-model §16.13, §16.14): what the Link menu would offer for this link BY
       * EXTENSION, so an unresolved link's applicable items are drawn disabled rather than absent.
       * Main sets both on every unresolved answer; absent only where main refused the request itself.
       */
...
export type LinkActionOutcome =
  ...
  | {
      readonly ok: false;
      readonly reason: 'gone' | 'refused' | 'unreachable';
      readonly path: string;
      /**
       * 045 FR-036, T229 (data-model §16.15): the OS's own words, set only when `reason` is `'refused'`
       * and the OS gave one. The one notice names them.
       */
      readonly osReason?: string;
    };

/**
 * 045 round four (data-model §16.18, §16.21, §16.22) — what `throng:links:follow` answers: ONE bounded
 * pass per Ctrl+click (FR-161), in which main has already performed any reveal itself, so the renderer
 * only ever has to open in throng or raise the one notice.
 *
 * - `openInThrong` — an existing in-project file; the renderer opens it by the click rule (FR-110).
 * - `revealed` — OS Explorer was asked: a folder as itself, a file's parent with it selected, or an
 *   out-of-project location that is not there on its parent (FR-158a, FR-158b, FR-160a).
 * - `notFound` — an in-project first reading with nothing behind it (FR-160): one notice, no editor.
 * - `unreachable` — an in-project first reading that did not answer in time (FR-124).
 * - `refused` — the OS declined, in its own words where it gave any (FR-036).
 * - `rejected` — the request failed I1's whitelist or main's `sanitiseLinkTarget` (FR-156). No gesture
 *   can build one, so the renderer raises nothing and logs a diagnostic.
 * - `failed` — the service threw; one failure notice names the path.
 */
export type LinkFollowOutcome =
  | { readonly kind: 'openInThrong'; readonly link: ResolvedLink }
  | { readonly kind: 'revealed' }
  | { readonly kind: 'notFound'; readonly path: string }
  | { readonly kind: 'unreachable'; readonly path: string }
  | { readonly kind: 'refused'; readonly path: string; readonly osReason?: string }
