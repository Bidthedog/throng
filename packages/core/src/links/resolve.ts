...
export function resolveCandidate(
  ...
  if (text.startsWith('/') || text.startsWith('\\')) {
    // FR-176 / FR-177: a drive form maps to its drive ONLY, in every flavour — the project-root
    // reading is not tried for it, so `/d/git/x.ts` is never the project's `d\git\x.ts`.
    const drive = driveFormOf(text, ctx.pathForms);
...
/**
 * FR-025 / FR-176, with FR-174's note: the platform's drive-form reading of `text`, or `null`.
 *
 * A drive form needs a separator AFTER its drive (`/<letter>/…`): a bare `/c` is a rooted path, so
 * `dir /c` never opens a drive. Which segment names the drive is the platform's knowledge, so the test
 * is asked of the port rather than spelled here: a text that names only a volume root — its reading
 * is the same with a separator appended — and does not end in one is not a drive form.
 */
function driveFormOf(text: string, pathForms: IPathForms): string | null {
  const drive = pathForms.fromDriveForm(text);
  if (drive === null) return null;
  const endsInSeparator = /[\\/]$/.test(text);
  if (!endsInSeparator && pathForms.fromDriveForm(`${text}/`) === drive) return null;
  return drive;
}
