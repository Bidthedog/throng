...
- [ ] T040 [P] [US5] E2E `packages/ui/tests/e2e/editor-naming.e2e.ts`: `foo.ts`→`foo`; open `bar.md`→`bar`; rename `Scratch`, open `baz.ts`→stays; "Reset Name"→`baz`; **"Reset Name" is disabled when the panel has never been renamed (FR-017)**; unsaved dot shows for auto-named and renamed; persists across restart.
