...
5. **Given** an **editor** panel, **When** a file/folder is dropped onto its content area, **Then** no path is pasted (editors are out of scope).
6. **Given** an **empty** (untyped) panel, **When** a **single file** is dropped, **Then** the open/convert behaviour (US4/#115) runs, not path-paste; a folder or multi-item drop is rejected there (US4).
...
7. **Given** any open menu or sub-menu, **When** the user presses **Escape** or clicks **outside**, **Then** the whole menu closes.
...
- **SC-002** (US2): Dropping a file or folder onto a terminal inserts its correct absolute path 100% of the time, a multi-item drag inserts every path space-separated in selection order, whitespace-containing paths arrive quoted, an editor drop pastes nothing, and an empty-panel drop still runs the open/convert path.
