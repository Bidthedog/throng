...
- **Slider controls for numeric values, and thousands separators in the displayed (never the stored) value** — issue **#53**. A genuine new control type across both Settings and Themes; it also extends the control vocabulary that feature 016's FR-022a then called exhaustive, so whichever of the two lands second must amend that sentence. *(2026-09-05, issue [#79](https://github.com/Bidthedog/throng/issues/79): both landed, and the amendment is done — 007 FR-028 now names `ControlKind` as the authority rather than enumerating the vocabulary, so a future control type no longer requires a spec edit.)*
