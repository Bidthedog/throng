...
- [x] T112 [P] [US6] RED component
  ...
  *Amended 2026-09-20 (round five, FR-180): "while web links and explicit `file:` hyperlinks still
  work" is withdrawn — an off switch hides every kind. T298 rewrites those cases.*
- [x] T113 [US6] Gate `provideLinks` in
  ...
  *Amended 2026-09-20 (round five, FR-180): the last sentence is withdrawn — the switch now gates every
  kind at four seams (T299).*
...
- [x] T165 [P] [US8] RED component `packages/ui/tests/component/editor-web-links.test.ts` — G11 – G14
  ...
  FR-103, SC-012.
  *Amended 2026-09-20 (round five, FR-180): the last case inverts — with the editor switch off a web
  link is not a link either (T298).*
...
| 16 Convergence (baseline, 2026-09-19) | Phase 15 | mapped into round four — see the table after Phase 16 |
| 17 D5 / marks from the view (round four) | Phase 16 mapping | D5's reproduction (T233) gates on the maintainer (T234) |
| 18 Space rule and known extensions | the RED table at `80e13719`; T240 – T247 need neither T236 nor the D5 gate, but T249 needs T237 and so **its checkpoint waits on the D5 gate** | the grammar every later phase scans with |
| 19 Classes, sanitising, allowlist, resolution | T242, T245 | classes read the grammar's spans |
| 20 Rendering without the disk | T237, T249, T239, T252 (T286) | the view pass and the options must exist before the old machinery goes |
| 21 Hint, wording, readout, Link menu | T252, T256, T263 | wording reads classes; the menu reads click-time resolution |
| 22 Docs, E2E, budget, gate | Phases 17 – 21 | E2E drives finished surfaces; T284 is the round's done-ness gate |
...
- [x] T225 *(Withdrawn by T265, research R32: the renderer link cache is deleted — nothing is resolved to draw a link (FR-155), so there is nothing to invalidate.)* Wire the renderer's existing file-watcher / file-changed broadcast to `invalidateLinksUnder(absPath)` (`packages/ui/src/renderer/links/link-cache.ts:122`, currently called only from unit tests), so a file created or deleted after its path was drawn gains or loses its link without waiting out the 30 s TTL, in terminals and editors, with a test at component layer or above proving the connection per FR-070 (partial)
- [x] T226 Bound `FileLinkResolver.locate` (`packages/ui/src/main/file-link-resolver.ts:171-197`) by one existence-check deadline per request rather than one per attempt, and give the terminal a way to honour an answer that arrives after `file-link-provider.ts`'s held reply (a detected-path equivalent of `pendingOscHover`, or re-asking xterm for the line), so a late answer becomes hoverable and Ctrl+clickable without moving the pointer per FR-123 (partial)
- [x] T227 Make following a link to a location that stops answering end within the existence-check timeout in total, across every reading and every root `locate` tries (shares T226's per-request deadline) per FR-124 (partial)
- [x] T228 Draw the solid hover underline on a hovered terminal link even when the idle scan has not collected it into `viewLinks` because output has not been quiet for `LINK_IDLE_SCAN_MS` (`packages/ui/src/renderer/terminal/link-marks.ts:197` returns an empty group), with a unit test hovering while output arrives per FR-135 / FR-131 (partial)
- [x] T229 Carry the OS's failure reason from `ElectronShellIntegration.openWithDefaultProgram` / `revealInFileManager` through `FileLinkResolver` (`file-link-resolver.ts:132-134`, `:154-156` discard it as a bare `refused`) into the one notice, so it names the file and the reason per FR-036 (partial)
- [x] T230 Reconcile the drive-form order: `resolve.ts:79` tries the project root before `fromDriveForm`, so `/d/git/x.ts` opens `<root>\d\git\x.ts` where one exists, while FR-024 puts the project root first only for a path that "is not a drive form" and FR-025 says a drive form "MUST map to that drive"; `link-resolve.test.ts:170-173` and `:367` assert the current order — settle which is intended (spec amendment or code + test change) per FR-024 / FR-025 (contradicts)
- [x] T231 Reconcile FR-151's WSL clause ("the project root and FR-025's `/mnt/<drive>` form only") with `resolve.ts:81-88` and `contracts/link-resolution.md:274` (R13), which still map Git Bash's `/<letter>/…` drive form in a WSL flavour — tighten the spec wording or restrict the WSL path to `/mnt/` with a test per FR-151 (contradicts)
- [x] T232 Correct the `editor.links.detectInEditors` descriptor (`packages/core/src/config/settings-metadata.ts:637`), which says Ctrl+click and Ctrl+Enter "keep their ordinary editor meanings everywhere" when off, though web links stay marked and followable (FR-101); reword to say only detected paths are affected, matching the terminal switch's wording per FR-060 / FR-061 (partial)

**Round-four mapping of Phase 16** *(2026-09-19; spec Clarifications, round four and planning; plan
round four; research R32)* — the tasks above are kept as written and are closed by these round-four
tasks, not built separately:

| Phase 16 | Status in round four | Closed by |
|---|---|---|
| T225 | **Withdrawn** — the renderer cache is deleted, nothing to invalidate (R32) | T265 records it |
| T226 | Main half kept; renderer half **moot** (no held reply) | T255 – T256 |
| T227 | Kept, unchanged (maintainer-approved) | T255 – T256 |
| T228 | **Is D5** | T233 – T239 |
| T229 | Kept (maintainer-approved) | T259 – T260 |
| T230 | Kept → FR-176 | T253 – T254 |
| T231 | Kept → FR-177, test only | T253 |
| T232 | Kept (maintainer-approved) | T246 – T247 |

---

## Phase 17: Round four — D5, marks drawn from the rows in view [US10] (FR-164, FR-172, SC-021, SC-026)

**Goal**: every well-formed link on screen is marked at rest within one throttle interval, even while a
full-screen program repaints continuously; hover and the hand pointer need no Ctrl and no prior scan; no
mark survives a buffer switch. **Independent test**: `terminal-link-view-marks.test.ts` green; quickstart
§10 rows 1 and 9.

- [x] T233 [US10] **D5 reproduction first (replicating-bugs gate).** Write
  `packages/ui/tests/unit/terminal-link-view-marks.test.ts` against today's
  `packages/ui/src/renderer/terminal/link-idle-scan.ts` + `link-marks.ts` over a fake terminal whose
  output **never goes quiet** (a write every 50 ms, as Claude Code's spinner) and which switches between
  the normal and alternate buffers: assert (a) the rows in view are marked within 100 ms of being drawn,
  (b) hovering a link that no pass has collected, **with Ctrl held**, draws the hover mark and the
  pointer class (the no-modifier pointer is FR-164's, T238 — so this reproduction isolates D5; spec D5's
  note on M11), (c) after a
  buffer switch no decoration from the other buffer remains. Run it, capture the RED output, and show it
  to the maintainer. **Touch none of D5's files until T234** — `link-idle-scan.ts`, `link-marks.ts`,
  `use-terminal.ts`, `terminal.css`. Other phases' production work (T236's constant declarations, Phase
  18's core grammar T240 – T247, Phase 19's core and main tasks) may proceed; T249's terminal half and
  everything built on T237 wait for the gate.
- [x] T234 [US10] The maintainer confirms T233 reproduces D5 (and T228). Record the confirmation under
  spec *Clarifications*, round four. Confirmed by the maintainer 2026-09-19: 'Yes, fix it' — T233 (f1bc6c81) reproduces D5, all four cases.
- [x] T235 [P] [US10] RED: extend `packages/core/tests/unit/link-guards.test.ts` with the edges of
  `LINK_MARK_THROTTLE_MS` (100) and `LINK_HINT_MS` (2,500), imported from `@throng/core`.
- [x] T236 [US10] GREEN for T235: add `LINK_MARK_THROTTLE_MS` and `LINK_HINT_MS` with JSDoc (plan
  Complexity Tracking, round four) to `packages/core/src/links/limits.ts`; export both from
  `packages/core/src/index.ts`.
- [x] T237 [US10] GREEN for T233 (a, c): create `packages/ui/src/renderer/terminal/link-view-marks.ts`
  per data-model §16.5 — `onRender`-driven, throttled at `LINK_MARK_THROTTLE_MS` with a guaranteed
  trailing run, `onBufferChange` → clear then immediate pass, scanning logical lines in view through
  `scanLinkLine` (no ask, no cache) **plus the OSC 8 source `oscLinksInView()`** (data-model §16.9 —
  xterm's OSC 8 ranges for the rows in view, so FR-136's OSC 8 at-rest mark survives the idle scan's
  removal; add an OSC 8 at-rest case to T233's file first) — and wire it in `packages/ui/src/renderer/terminal/use-terminal.ts`
  in place of the idle-scan wiring at `:1117-1149` (the `subscribeLinkCache` / `pendingOscHover` re-arm at
  `:1150-1160` goes with T265 and T286). Depends on T234 (the D5 gate) and T236.
- [x] T238 [P] [US10] RED: in `packages/ui/tests/unit/terminal-link-affordance.test.ts`, assert the hand
  pointer class is set on hover **without** the modifier (FR-164) and that the hover mark comes from the
  provider's hovered range; and in `packages/ui/tests/component/link-decorations.test.ts`, that the
  editor's link mark carries `cursor: pointer` without the modifier.
- [x] T239 [US10] GREEN for T233 (b) and T238: `packages/ui/src/renderer/terminal/link-marks.ts` draws
  the hover mark from the hovered link itself (`:157`, `:197` stop consulting `viewLinks` for it) and
  drops the Ctrl condition on `LINK_MARK_POINTER_CLASS` (`:170`); `packages/ui/src/renderer/terminal/terminal.css`
  and the editor link-mark style in `packages/ui/src/renderer/editor/link-decorations.ts` show
  `cursor: pointer` unconditionally. T233 and T238 green. Depends on T237.

**Checkpoint**: D5's reproduction is green; the idle scan is no longer wired (deleted in T264).

---

## Phase 18: Round four — the space rule, rooted paths and the known-extension setting [US12, US13] (FR-155, FR-173, FR-174, FR-178, FR-178a, FR-179, SC-022)

**Goal**: the committed RED table (`link-detect-spaces.test.ts`, `80e13719`) is green; the known-extension
set is a live user setting stored as edits. **Independent test**: the table and quickstart §10 row 10.

- [x] T240 [P] [US12] RED: create `packages/core/tests/unit/known-extensions.test.ts` for
  `resolveKnownExtensions(shipped, { added, removed })` — trim, lower-case, one leading dot stripped,
  empties dropped, `(shipped ∪ added) − removed`, `*` in `removed` empties the shipped half, an addition
  survives `*`, result is a fresh set (FR-178a).
- [x] T241 [US12] GREEN for T240: create `packages/core/src/links/known-extensions.ts` with
  `KNOWN_FILE_EXTENSIONS` (lower-case, no dot; at least every member `link-detect-spaces.test.ts`'s
  `KNOWN_FILE_EXTENSIONS` block pins) and `resolveKnownExtensions`; export from
  `packages/core/src/index.ts`. The guard `packages/core/tests/unit/links-no-os-names.test.ts` already
  scans `core/src/links/**`; give it a **named exemption** for `known-extensions.ts` citing FR-178b (the
  list names `exe`, `ps1` … as a hint for where a path ends, not an executability rule) — every other
  file stays under the full rule.
- [x] T242 [US12] [US13] GREEN for the committed RED `packages/core/tests/unit/link-detect-spaces.test.ts`:
  implement `detectPathSpans(line, { knownExtensions })` in `packages/core/src/links/detect.ts` exactly
  per that file's header rules 1 – 5 (enclosures incl. `{}` and `'…'`; the forward scan from any anchored
  or separator-bearing candidate to the **first** separator- or known-extension-ending word; single
  spaces only; `MAX_PATH_SPACE_WORDS` capping the **whole span**, FR-179f). **Rebuild
  `detectPathCandidates` on this rule** (it keeps returning `LinkCandidate`s for `scanLinkLine`) and make
  `detectPathSpans` a view over it (plan *Corrections, second pass*); make any rooted path a link, with
  FR-174's note's path characters — add `/?` (not a link) and `dir /s` (`/s` is) to `link-detect.test.ts`,
  plus FR-179f's edge — a span of exactly 6 words is one link (`link-detect.test.ts`, not the confirmed
  table) (`/help`, FR-174); **remove** round
  three's longest-first extended readings. Clear the test file's "PROPOSED CASES, FOR REVIEW" header and
  its `unsure` markers, citing the confirmation recorded in spec *Clarifications* (planning block). Rewrite the extended-reading cases in
  `packages/core/tests/unit/link-detect.test.ts` to the new rule (permitted by FR-173/FR-179; the table
  wins every edge). Depends on T241.
- [x] T243 [US13] Re-check SC-003's prose fixture against FR-174 (`packages/core/tests/unit/link-detect.test.ts`
  and the fixture file it reads): it must yield zero spans by grammar alone, or hold no rooted token;
  if it holds one, change the fixture's **expectation** to a link, citing FR-174, never the grammar.
  Run the fixture through `scanLinkLine` (the function rendering uses), not only `detectPathCandidates`.
  Depends on T242, T245.
- [x] T244 [P] [US13] RED: create `packages/core/tests/unit/link-scan-line-options.test.ts` — `scanLinkLine(line,
  { knownExtensions, allowlist, refused })` passes the extension set through to `detectPathSpans`, returns
  allowlisted protocol spans (`mailto:a@b.c`, `tel:+44…`, `slack://open`) as their own kind, never one
  in `refused` (defaulting to core's half) even when allowlisted, and keeps FR-009's "never both"; FR-159a's
  grammar: `mailto:a@b.c.` → `mailto:a@b.c`, `slack:open` and `slack://open` both links, `tel:+44 20` →
  `tel:+44`, `mailto:` alone not a link, a non-allowlisted `foo:bar` not a link.
- [x] T245 [US13] GREEN for T244: `packages/core/src/links/scan-line.ts` takes the options, with the
  protocol grammar in a **new** `packages/core/src/links/protocol-uri.ts` per FR-159a (`web-url.ts` and
  `link-web-url.test.ts` unchanged); create `packages/core/src/links/refused-schemes.ts` (core's
  OS-neutral half — `javascript`, `data`, `vbscript`, `about`, `blob`; **not** `file`) here so the
  `refused` default exists; callers without options get the shipped defaults. Depends on T242.
- [x] T246 [P] [US13] RED: extend `packages/core/tests/unit/app-settings.links.test.ts` (defaults and
  tolerant parse of `editor.links.protocolAllowlist` = `['mailto','tel','slack']` and
  `editor.links.knownFileExtensions.added`/`.removed` = `[]`; non-arrays fall back, non-strings dropped)
  and `packages/core/tests/unit/settings-metadata-links.test.ts` (three descriptors under Editor · Links,
  `control: 'array'`, `itemControl: 'text'`, `clearable: true`; the `removed` description lists every
  `KNOWN_FILE_EXTENSIONS` member; the extension and allowlist descriptors say they apply to terminals
  **and** editors (FR-178); Reset to Defaults empties `added`/`removed`, restoring the shipped list
  (FR-178a); **T232**: `detectInEditors`/`detectInTerminals` descriptions contain no
  "exists" and say web links stay followable; the timeout description names following a link and
  opening the Link menu, FR-161; and **FR-060a** — the switch descriptors say allowlisted protocol
  links, like web links, are unaffected); **FR-159b** — allowlist entries `slack:`, ` Slack ` and
  `zoommtg://` normalise to `slack` / `zoommtg`, an empty entry is ignored.
  *Amended 2026-09-20 (round five): three of these expectations invert — the extension leaves become one
  list (FR-182, T297), the switch descriptors say every kind is governed rather than exempting web and
  protocol links (FR-180b, T299), and the timeout's label becomes **Link resolution timeout** (FR-181,
  T296). The `KNOWN_FILE_EXTENSIONS`-listing description goes with the `removed` leaf.*
- [x] T247 [US13] GREEN for T246: `packages/core/src/config/app-settings.ts` and
  `packages/core/src/config/settings-metadata.ts` (serial — both files). `settings-metadata.test.ts`'s
  completeness guard must pass unchanged. Closes T232. Depends on T241.
- [x] T248 [US13] RED: extend `packages/ui/tests/component/link-setting-live.test.ts` — adding `foo` to
  `knownFileExtensions.added` or `mailto` to/from the allowlist changes the next terminal view pass and
  the next editor decoration pass with no remount (FR-159, FR-178).
- [x] T249 [US13] GREEN for T248: *(round five: the feed is now `knownFileExtensionsSet(links.knownFileExtensions)`
  through `link-scan-options.ts` — FR-182b, T297)* feed `resolveKnownExtensions(KNOWN_FILE_EXTENSIONS, links.knownFileExtensions)`
  and the allowlist into `scanLinkLine` from `packages/ui/src/renderer/terminal/terminal-panel.tsx`
  (`linkSettingsRef`, `:287`), `packages/ui/src/renderer/terminal/use-terminal.ts` (`linkSettings`) and
  `packages/ui/src/renderer/editor/use-editor.ts` / `link-decorations.ts`. The **editor half** depends
  only on T245, T247 and may land before the D5 gate; the **terminal half** depends on T237 (and so on
  T234).

**Checkpoint**: SC-022 — the maintainer-verified table is 100% green.

---

## Phase 19: Round four — resource classes, sanitising, the allowlist and click-time resolution [US13] (FR-156 – FR-161, FR-176, FR-177, SC-021, SC-024; T226, T227, T229 – T231)

**Goal**: a Ctrl+click goes where its class says; nothing crafted runs; the disk is touched only at a
follow or a Link-menu opening, bounded in total. **Independent test**: quickstart §10 rows 3 – 6.

- [x] T250 [P] [US13] RED: create `packages/core/tests/unit/link-sanitise.test.ts` — SC-024's crafted
  fixture (`file:///C:/x.txt%00.exe`, `https://x" --foo`, embedded C0/C1, raw NUL, `&|;` metacharacters,
  quote-trailing text) is refused or reduced to one `URL`-serialised value, and surrounding whitespace is
  trimmed (FR-156); **FR-156b** — `D:\R&D\notes.txt` and a path with `;` or `^` are kept whole and
  accepted (metacharacters are never a refusal reason; only controls, NULs and quote-trailing text are).
- [x] T251 [P] [US13] RED: create `packages/core/tests/unit/link-resource-class.test.ts` — FR-157's
  table (web; loopback for `localhost`, `127.0.0.1`, `[::1]`; UNC in all three spellings; on-device incl.
  `file:` and plain paths; protocol only when allowlisted), refused wins over the allowlist for every
  member of the core refused set, case-insensitive schemes, empty target → `null`; a `file:` URI is
  `onDevice` and `file` is **not** in the set classification reads (plan *Corrections after analysis*).
- [x] T252 [US13] GREEN for T250 – T251: create `packages/core/src/links/sanitise.ts` and
  `packages/core/src/links/resource-class.ts` (reading T245's `refused-schemes.ts`);
  call `sanitiseLinkTarget` from `scan-line.ts`; export from `packages/core/src/index.ts` (the guard `links-no-os-names.test.ts` already scans them — no edit).
  Depends on T245.
- [x] T253 [P] [US13] RED: in `packages/core/tests/unit/link-resolve.test.ts`, **FR-176** — `/d/git/x.ts`
  resolves to `D:\git\x.ts` only, even with a project holding `d\git\x.ts` (change `:170-173` and `:367`,
  which assert the old order; permitted by FR-176, noted in *Notes*); **FR-177 / T231** — in a WSL
  flavour `/c/x` and `/mnt/c/x` both map to `C:\x` (test only; the code already does); **FR-174's
  note** — a bare `/c` with no following separator is a rooted path, never a drive form.
- [x] T254 [US13] GREEN for T253: `packages/core/src/links/resolve.ts:79` — a drive form maps to its
  drive and the project-root reading is not tried for it. Closes T230, T231. Depends on T242.
- [x] T255 [P] [US13] RED: extend `packages/ui/tests/integration/file-link-resolver.integration.test.ts`
  and `packages/ui/tests/unit/file-link-resolver-network.test.ts` — one follow whose every reading and
  root hangs ends within `existenceCheckTimeoutMs` **in total** (T226 main half, T227); an in-project
  first reading with nothing existing yields, on `throng:links:resolve`, the failure arm with no
  `reason` (FR-160; data-model §16.17 — the `notFound` **follow** outcome is T293's); **FR-122a** — after a
  root's stuck check settles, a further check under it within `LINK_ROOT_BACKOFF_MS` answers `unreachable`
  without touching the disk, and one after it checks again (in `file-link-resolver-network.test.ts`, fake
  clock); an out-of-project location is
  handled per FR-158a — an existing folder with no trailing separator (`\\server\share\dir`) opens **as
  itself**, an existing file opens its parent with it selected, an absent or unreachable one goes to its
  parent with no notice, all within the one deadline — and a trailing-separator location or an OSC 8
  target ending in one opens as a folder with **no** `stat` call — and, **FR-158c**, a relative `sub\dir\`
  from a working directory outside the project reveals the **working-directory** (first) reading, with no
  `stat`; the **mixed list** (FR-160's note):
  `/etc/hosts` with `<root>\etc\hosts` absent and Git's `etc\hosts` present reveals Git's file, and
  `/help` with nothing behind it raises the one notice; a `file:///` in-project target opens in
  throng; **FR-160a** — an existing in-project **folder** that is not the root (`<root>\src`) opens as
  itself in OS Explorer after one `stat`, and **FR-158d** — a follow of the project root itself
  (`file:///D:/proj`, no trailing slash — SC-005's link) opens as itself with **0** `stat` calls;
  **FR-122a** — two roots in back-off do not gate a third, healthy share (the back-off map does not count
  toward `MAX_TIMED_OUT_LINK_CHECKS`); a folder by grammar under a root in back-off still opens as a
  folder with no check (FR-122a's note, FR-158d wins); and an in-project first reading that times out yields the `unreachable` outcome
  for FR-124's "did not answer" notice, not "not found"; SC-024's crafted fixture (`"C:\a.txt" --flag`, `%00`, controls) sent to reveal, open-in-throng,
  default program and Open Program is refused or reduced to one sanitised path — main re-sanitises every
  request (FR-156); **FR-158b / S10** — `throng:links:reveal` (the explicit Open in OS Explorer item) on a
  missing out-of-project location reveals its parent instead of answering `gone`: rewrite
  `packages/ui/tests/integration/link-ipc-confinement.integration.test.ts:197-207` and
  `file-link-resolver.integration.test.ts:310-327` accordingly, keeping every confinement case; the
  menu-open reply for an **unresolved** link carries `executableByExtension` and
  `previewByExtension` (data-model §16.13, FR-170a); and in
  `packages/ui/tests/integration/link-de-elevated-open.integration.test.ts`, **Open Program** on an
  executable goes through `openWithDefaultProgram` and the de-elevating launcher (FR-038's extension).
- [x] T256 [US13] GREEN for T255: `packages/ui/src/main/file-link-resolver.ts` — one deadline per request
  in `locate` (`:171-197`) returning data-model §16.10's outcomes (not-found is the failure arm with no
  `reason`, §16.17; the follow's `LinkFollowOutcome` is T294's), FR-158a's file-or-folder outcome (drop the reveal policy's separate existence
  re-check; reveal-as-itself for a folder), FR-160's first-existing-reading rule and not-found split, and
  `sanitiseLinkTarget` on every request; FR-122a's per-root back-off with `LINK_ROOT_BACKOFF_MS` (30,000)
  added to `packages/core/src/links/limits.ts` and exported from `packages/core/src/index.ts`; the unresolved
  reply's by-extension fields, through `packages/ui/src/main/link-ipc.ts` where its shape passes; an Open Program
  action routed through `openWithDefaultProgram` (plan *Corrections, fifth pass*). Closes T226 (main
  half) and T227. Depends on T254, T252 (`sanitiseLinkTarget`).
- [x] T257 [P] [US13] RED: create `packages/core/src/testing/refused-uri-schemes-contract.ts` (data-model
  / contracts `platform-ports.md` §7.1) and `packages/platform-windows/tests/contract/windows-refused-uri-schemes.contract.test.ts`
  running it plus **every** entry of research R27's Windows list present (`ms-msdt`, `search-ms`,
  `search`, `ms-officecmd`, `ms-appinstaller`, `ms-cxh`, `ms-cxh-full`), so dropping one fails a test; and create `packages/ui/tests/unit/external-link-uri.test.ts`
  — `isAllowedLinkUri` accepts http(s), loopback and allowlisted schemes, refuses every refused scheme
  even when allowlisted, refuses `file:` always (FR-037), re-sanitises; the handler for
  `throng:linkUri:openExternal` reads the allowlist per request. `packages/ui/tests/unit/external-url.test.ts`
  **must not change**.
- [x] T258 [US13] GREEN for T257: create `packages/core/src/abstractions/refused-uri-schemes.ts`
  (`IRefusedUriSchemes`) + its DI token; `packages/platform-windows/src/windows-refused-uri-schemes.ts`
  (research R27's list with sources) exported from `packages/platform-windows/src/index.ts`; bind it in
  `packages/ui/src/main/composition-root.ts`; add `isAllowedLinkUri` and the `throng:linkUri:openExternal`
  registration to `packages/ui/src/main/external-url.ts` through a **new** function
  `registerLinkUriIpc(ipc: { on; handle }, refused: IRefusedUriSchemes, readAllowlist)` — the existing
  `registerOpenExternalIpc` and its `{ on }`-only `OpenExternalIpcMain` keep their signatures, so
  `external-url.test.ts` stays unchanged — called from `packages/ui/src/main/main.ts` beside the existing
  registration (`:988`); existing predicates untouched; expose it in
  `packages/ui/src/preload/preload.cts` and `packages/ui/src/renderer/global.d.ts`; route web, loopback and
  protocol follows through it from `packages/ui/src/renderer/links/link-actions.ts` (`:161`). Depends on
  T252.
- [x] T259 [P] [US13] RED: extend `packages/core/src/testing/shell-integration-contract.ts` (a failing
  reveal / default-program open resolves `{ ok: false, osReason }` with a non-empty `osReason`),
  `packages/ui/tests/integration/electron-shell-integration.test.ts` and
  `packages/ui/tests/component/link-failure-notice.test.ts` (the one notice names the file **and** the OS's
  reason, FR-036; and FR-160's not-found case — an in-project link with nothing behind it raises exactly
  one notice naming the path and opens no editor or preview).
- [x] T260 [US13] GREEN for T259: `packages/core/src/abstractions/shell-integration.ts`,
  `packages/ui/src/main/electron-shell-integration.ts`, `packages/ui/src/main/file-link-resolver.ts`
  (`:132-134`, `:154-156` carry `osReason`), `packages/core/src/links/types.ts` (`osReason?` on the
  refused arm — `reason` is the arm's discriminant), `packages/ui/src/renderer/links/link-actions.ts`. Closes T229. Depends on T256 (same file).
- [x] T287 [P] [US13] *(added after analysis)* RED: create
  `packages/ui/tests/unit/refused-schemes-client.test.ts` — before main answers, the client returns core's
  refused half; after `throng:linkUri:refusedSchemes` answers, the union; asked once per window; and in
  `packages/ui/tests/component/link-setting-live.test.ts`, an allowlisted `ms-msdt:x` is **not** drawn as a
  link once the platform set has arrived (FR-159, US13 scenario 8; research R34).
- [x] T288 [US13] GREEN for T287: register `ipcMain.handle('throng:linkUri:refusedSchemes')` over
  `IRefusedUriSchemes` inside T258's `registerLinkUriIpc` in `packages/ui/src/main/external-url.ts` (wired
  from `packages/ui/src/main/main.ts`); expose it in
  `packages/ui/src/preload/preload.cts` and `packages/ui/src/renderer/global.d.ts`; create
  `packages/ui/src/renderer/links/refused-schemes-client.ts`; pass its set into every `scanLinkLine` call
  T249 wires. Depends on T258, T249.
- [x] T289 [P] [US13] *(added after the fourth analysis — one scheme gate)* RED: rewrite
  `packages/core/tests/unit/link-classify.test.ts` (`:26-30`'s `mailto:` → inert becomes: allowlisted →
  protocol, else not a link) against `resourceClass`, and add to
  `packages/ui/tests/unit/terminal-link-activation.test.ts` an OSC 8 `mailto:` Ctrl+click that reaches
  `throng:linkUri:openExternal` when allowlisted and does nothing when not (FR-159, FR-163, US13 scenario 7).
- [x] T290 [US13] GREEN for T289: retire `classifyTerminalLinkTarget` from
  `packages/core/src/links/classify.ts` into `resourceClass` (or make it a one-line delegate, then delete
  it once no caller remains) and move every call site onto `resourceClass`:
  `packages/ui/src/renderer/terminal/terminal-link-activation.ts` (`:137`, `:167` —
  `activateTerminalHyperlink` gains the protocol route), `packages/ui/src/renderer/terminal/link-marks.ts:142`,
  `packages/ui/src/renderer/links/link-actions.ts:160`, `packages/ui/src/renderer/terminal/use-terminal.ts`
  (`:507`, `:571`, `:1108`), and `packages/core/src/terminal/link-menu.ts:34` unless T277 has already
  deleted it. Depends on T252, T258, T237; the final deletion of `classifyTerminalLinkTarget` (and of
  `classify.ts` if nothing else remains in it) waits on T277, which also updates
  `packages/core/tests/unit/links-no-os-names.test.ts:151-165`'s module list.
- [x] T291 [P] [US13] *(added after the fifth analysis — FR-060a and the channel move)* RED: in
  `packages/ui/tests/component/link-detection-switches.test.ts` and
  `packages/ui/tests/component/editor-web-links.test.ts` (`:116` and its channel assertion): web and
  protocol follows reach `throng:linkUri:openExternal` (not `throng:openExternal`); with detection **off**
  an allowlisted `mailto:` is still marked, followed and hinted, while detected paths are not; with
  `mailto` allowlisted, `:116`'s editor `mailto:` is a link (S5). GREEN: T258 (channel), T249 and T271
  (marking and hint). Write before T258.

**Checkpoint**: SC-024 green; every class routes per FR-157.

---

## Phase 20: Round four — rendering without the disk; dead code removed (FR-155, SC-021)

**Goal**: no surface asks about existence to draw, mark, hover, word or read out a link.
**Independent test**: SC-021's zero-check counter in `link-parity.test.ts`.

- [x] T261 [P] [US13] RED: extend `packages/ui/tests/unit/link-parity.test.ts` with SC-021's counter — a
  fake resolver counting calls across provide/hover/view-pass/tooltip/readout for every link kind and
  class in both panel types, with output that never pauses: **0** calls; then, for a UNC or on-device
  link only, exactly 1 `throng:links:follow` request per Ctrl+click and **at most** 1
  `throng:links:resolve` per Link-menu opening — for a folder by grammar (a trailing separator, an OSC 8
  folder target or the project root) still 1 `follow` (main must reveal it) but 0 `resolve` at a menu
  opening, its "no check" being main's 0 `stat` calls, asserted in T255 (FR-158d) — and **0** for web, loopback
  and protocol links (SC-021's note).
- [x] T262 [US13] GREEN: `packages/ui/src/renderer/terminal/file-link-provider.ts` answers
  `provideLinks` synchronously from `scanLinkLine` — delete the held reply, `LINK_ANSWER_DEADLINE_MS`
  (`:167`), superseding and the detected-path hover `ask`; rewrite
  `packages/ui/tests/unit/terminal-file-link-provider.test.ts` (held-reply/deadline cases deleted,
  synchronous-answer cases added). Depends on T249, T294.
- [x] T293 [P] [US13] *(added after the ninth analysis — the follow path)* RED: in
  `packages/ui/tests/component/link-actions-router.test.ts` and `link-position-open.test.ts`, a Ctrl+click
  on a detected path or a `file:` link with **no** prior answer sends **one** `throng:links:follow` and
  acts on its `LinkFollowOutcome` (data-model §16.18): `openInThrong` → opens in throng by the click rule
  (position kept); `revealed` → nothing more; `notFound` → FR-160's one notice; `unreachable` → FR-124's
  "did not answer" notice; `refused` → FR-036's notice with `osReason`; `failed` → exactly one failure notice
  naming `path`; `rejected` → no notice and one logged diagnostic — in terminal and editor alike, and
  exactly one main request per follow (plan *Corrections, tenth pass*). In
  `packages/ui/tests/integration/file-link-resolver.integration.test.ts`, `FileLinkResolver.follow`
  performs the reveal itself for a folder or out-of-project file and replies `revealed`, within one
  deadline — and T255's follow-shaped cases are re-asserted **through `follow`** (the hanging total
  deadline, the project root with 0 `stat` calls, the back-off, the `/etc/hosts` and `/help` mixed list,
  FR-158c's relative folder), SC-024's crafted fixture sent to `follow` is refused or reduced to one
  sanitised path — a refused one answers `{ kind: 'rejected' }` — a request failing I1's whitelist (text,
  kind, panelId) answers `{ kind: 'rejected' }`, a follow with an **unknown or absent**
  `originProjectId` is judged out-of-project (I2 / M3) and reveals rather than answering `rejected`, and a
  throwing service answers `{ kind: 'failed'; path; message }` (data-model §16.21 – §16.22); the working-directory base of a terminal's
  follow request is the directory **at the moment of the click** (FR-023 / FR-144 — the property
  `terminal-link-base-directory.test.ts:108-127` pinned on the hover ask; the assertion moves into that
  same file, re-aimed at the follow request, and its `askTerminalLink` import at `:10` goes); and in `packages/ui/tests/contract/link-ipc.contract.test.ts` — four `handle` channels
  (`open`, `resolve`, `reveal`, `follow`; `:69-72`), `follow` held to the I1/I2 whitelist and root
  derivation, preload and `global.d.ts` parity for `window.throng.links.follow` (`:220-226`), and the
  `LinkIpcService` literals at `:39` and `:248` (and the inline services at `:111-115`, `:121-131`) gaining
  `follow`, and the "three channels" / "all three" titles at `:65`, `:211`, `:220`, `:247` retitled to four. The no-`send` (`:216`) and invoke-only
  (I6) invariants stay: the allowlisted-scheme channels are `throng:linkUri:*`, outside this namespace.
- [x] T294 [US13] GREEN for T293: add `LinkFollowOutcome` (with its `rejected` and `failed` arms) to
  `packages/core/src/links/types.ts`; `follow` applies `sanitiseLinkTarget` to every request (FR-156);
  `FileLinkResolver.follow` in `packages/ui/src/main/file-link-resolver.ts` and its `throng:links:follow`
  registration in `packages/ui/src/main/link-ipc.ts` (its JSDoc at `:7`, "Three channels, all `invoke`",
  becomes four; the `LinkIpcService` interface and the resolve /
  reveal / open / follow handlers live there; `throng:linkUri:openExternal` and
  `throng:linkUri:refusedSchemes` stay in `external-url.ts`, T258 / T288) (with `packages/ui/src/preload/preload.cts`, `packages/ui/src/renderer/global.d.ts`); make
  `followLink` in `packages/ui/src/renderer/links/link-actions.ts` (`:285-294`) async over it, dropping its
  synchronous `resolve` argument; update its callers in
  `packages/ui/src/renderer/terminal/terminal-link-activation.ts` and
  `packages/ui/src/renderer/editor/use-editor.ts`, and the test callers
  `packages/ui/tests/component/link-default-action-wiring.test.ts` (`:161`, `:184`, `:236`, `:319`, `:348`,
  `:357`), `packages/ui/tests/component/link-executable-refusal.test.ts` (`:124`, `:145`, `:162`),
  `packages/ui/tests/component/link-out-of-project.test.ts` (`:155`, `:174`, `:275`),
  `packages/ui/tests/component/link-setting-live.test.ts` (`:111`, `:142`, `:207`, `:209`) and
  `packages/ui/tests/unit/link-parity.test.ts:210` —
  their "a gesture never runs an executable" assertions stay; reshape `HoveredLink`
  (`packages/ui/src/renderer/terminal/hovered-link.ts`) to drop its resolved `link`. Depends on T256,
  T260. Must land before T262, T263, T265 and T286 remove the answers the old path read.
- [x] T263 [US13] GREEN: `packages/ui/src/renderer/editor/link-decorations.ts` and
  `packages/ui/src/renderer/editor/use-editor.ts` mark by grammar alone; rewrite the cases in
  `packages/ui/tests/component/link-decorations.test.ts`, `editor-link-gestures.test.ts`,
  `editor-web-links.test.ts` and `link-position-open.test.ts` that expected an unresolved path unmarked.
  Depends on T249's editor half and T294. It shares `link-decorations.ts` with T239 only for serialisation: if the
  D5 gate is still pending, T263 goes first and T239's editor-style line follows.
- [x] T264 [US13] Delete `packages/ui/src/renderer/terminal/link-idle-scan.ts`,
  `packages/ui/tests/unit/terminal-link-idle-scan.test.ts` and `LINK_IDLE_SCAN_MS` (limits.ts, core
  index), and the idle-interval `describe` block in `packages/core/tests/unit/link-guards.test.ts`
  (`:95-114`); move its still-true assertions (never reads output data; per-line cap) into
  `terminal-link-view-marks.test.ts`. Depends on T237.
- [x] T265 [US13] Delete `packages/ui/src/renderer/links/link-cache.ts`, `packages/ui/tests/unit/link-cache.test.ts`,
  `LINK_CACHE_TTL_MS` and its edge in `link-guards.test.ts`; remove every import (`terminal-link-activation.ts`,
  `use-terminal.ts`, `link-marks.ts`, `use-editor.ts`, `link-decorations.ts`, and the **unit and component**
  tests `git grep link-cache` lists — including `terminal-link-base-directory.test.ts:3`, `:70`,
  `terminal-link-activation.test.ts` and `terminal-link-affordance.test.ts`). Correct the JSDoc of `MAX_LINK_CANDIDATES_PER_LINE`, `MAX_PATH_SPACE_WORDS`
  (the whole span, starting token included — FR-179f) and `MAX_TIMED_OUT_LINK_CHECKS` to what they bound
  now. Tick **T225** as withdrawn with a note citing
  R32. Depends on T262, T263, T286, T294. T261 green.
- [x] T285 [P] [US13] *(added after analysis — FR-163)* RED: in
  `packages/ui/tests/unit/terminal-link-affordance.test.ts` and `terminal-link-view-marks.test.ts`, an
  OSC 8 `file:` target that does not exist is marked at rest, hovered, pointed and worded **without any
  call to main**; an OSC 8 target of an allowlisted scheme is a link; an empty, refused or
  non-allowlisted target still presents no affordance (FR-154's surviving cases); an OSC 8 target carrying
  control characters or `%00` is not drawn clickable (FR-156's render-side half, data-model §16.15). Rewrite the FR-154
  cases that expected an unreachable `file:` target to look like text (permitted by FR-163), and in
  `packages/ui/tests/unit/terminal-hovered-link.test.ts` the reader-3/reader-4 blocks (`:189-224`,
  `:240-246` — `file:` gone → null, `mailto:` → null, the resolved `link` carried) and
  `link-detection-switches.test.ts:182`'s hover ask (FR-159, FR-163).
- [x] T286 [US13] GREEN for T285: judge OSC 8 targets by `resourceClass` alone in
  `packages/ui/src/renderer/terminal/use-terminal.ts` (`:501-514`, `:1107-1110`) and
  `packages/ui/src/renderer/terminal/hovered-link.ts`; delete `pendingOscHover` and the ask in
  `hoveredLinkFromUri` (which lives in `packages/ui/src/renderer/terminal/terminal-link-activation.ts:131`,
  including its hover-time `drawn` resolution, withdrawn by FR-163); feed `oscLinksInView()` (T237) from
  xterm's OSC 8 ranges, each target through `sanitiseLinkTarget` first (data-model §16.15). Depends on
  T237, T252, T290, T294.
- [x] T292 [US13] *(added after the seventh analysis)* Bring the existing E2E in line with FR-155 /
  FR-163, **inside its existing declarations** (no budget change): in
  `packages/ui/tests/e2e/terminal-links.e2e.ts`, the look-alike `./nothere.ts` (`:66`, `:84-92`, `:213-307` — including the at-rest
  `not.toContain` pair at `:298-307`, which now becomes `toContain`) and the two
  dead OSC 8 targets `file:///C:/does/not/exist.txt` and `file://nonexistent-host-xyz/share/file.txt`
  (`:284-296`, `:370-452`) now assert a mark, the hand pointer and an underline; `notascheme:foo` and the
  empty target still assert none; the header comment (`:12-40`) and the comments at `:156`, `:239`, `:295`, `:313`, `:374` drop the
  idle-scan and FR-006 wording; retitle the declaration at `:153` ("…a look-alike is not…" → it is marked
  too, and "a dead hyperlink on none" → "an unfollowable hyperlink on none"), keeping it on one line with its tags — the budget counts declarations by tag, not title.
  Reason at this layer (Principle V): only a real xterm draws the OSC 8 underline and pointer. Depends on
  T262, T265, T286, T239.

**Checkpoint**: SC-021 holds; `git grep -n "LINK_IDLE_SCAN_MS\|LINK_CACHE_TTL_MS\|LINK_ANSWER_DEADLINE_MS\|link-cache\|link-idle-scan" -- packages`
returns nothing.

---

## Phase 21: Round four — the link hint, the wording, the readout and the Link menu [US14, US15, US16] (FR-165 – FR-171, FR-175, S6, S7, SC-023, SC-025, SC-027)

**Goal**: one hint, one wording function, one readout, one Link menu, identical in terminal, editor and
Markdown preview. **Independent test**: quickstart §10 rows 2, 7, 8, 11.

- [x] T266 [P] [US14] RED: extend `packages/core/tests/unit/link-hover-text.test.ts` with FR-168's table
  (contracts `menus-and-gestures.md` §9.4): active/new editor from `editor.openTarget`, preview, OS
  Explorer, system browser, `<scheme>` handler; destination by name (research R30); and the preview's
  destinations (plan *Corrections, third pass*; data-model §16.11): `heading` → "Ctrl+Click to go to the
  heading", `stays` → "Ctrl+Click to follow"; **FR-168b** — the project root itself and a path ending in a
  separator are worded "Ctrl+Click to show in OS Explorer", with no disk access; **FR-168a** — a relative
  terminal path whose working directory lies outside the project is worded from its working-directory
  reading ("…to show in OS Explorer"); a Markdown link **with a position** (`README.md:12`) whose default
  open action is Preview is worded "…active editor" / "…new editor", because a position opens an editor
  (FR-110, SC-016).
- [x] T267 [US14] GREEN for T266: `packages/core/src/links/hover-text.ts` (`linkHoverText` takes the
  destination — editors and terminals derive it from FR-157 / FR-160 by name, the preview from 044's
  follow, S6); update its callers `packages/ui/src/renderer/terminal/hovered-link.ts`,
  the editor tooltip in `packages/ui/src/renderer/editor/link-decorations.ts`, and the Markdown preview's
  tooltip in `packages/ui/src/renderer/preview/link-dom.ts` (FR-168's note), with RED cases first in
  `packages/ui/tests/component/preview-links.test.ts` and `packages/ui/tests/unit/terminal-hovered-link.test.ts`
  (`:145`, `:172` — the new wording). Depends on T252.
- [x] T268 [P] [US14] RED: create `packages/ui/tests/component/link-hint.test.tsx` — shown on a plain
  click with the FR-168 text; anchored at the last row's bottom-right; clamped on screen at right and
  bottom edges through `clampToViewport`; `pointer-events: none`, not focusable; hidden by Ctrl keydown,
  a link Ctrl+click, a second hint, a **user** scroll, window blur and after `LINK_HINT_MS` (fake timers),
  and **not** hidden while output streams and scrolls the terminal (FR-165d's note); never on
  hover, Ctrl+click or drag; none where that kind is disabled (FR-165f); at most one.
- [x] T269 [US14] GREEN for T268: create `packages/ui/src/renderer/links/link-hint-store.ts` and
  `packages/ui/src/renderer/links/link-hint.tsx` (reusing `packages/ui/src/renderer/common/clamp-to-viewport.ts`,
  no copy) and `packages/ui/src/renderer/links/link-hint.css`; mount once in
  `packages/ui/src/renderer/composition-root.tsx`. Depends on T267.
- [x] T270 [US14] Theme tokens `linkHintBackground`, `linkHintText`, `linkHintBorder`: RED in
  `packages/core/tests/unit/theme-link-tokens.test.ts` (descriptor, parent, present in every default
  theme; parents `surfaceActive`, `text`, `border` — plan *Corrections, fifth pass*) and
  `packages/core/tests/unit/shipped-defaults-upgrade.test.ts` (version 11 materialises them
  additively, idempotent re-run); GREEN in `packages/core/src/config/theme.ts`, `theme-metadata.ts`,
  `default-themes/index.ts`, `theme-copy.ts` and `shipped-defaults.ts` (`SHIPPED_DEFAULTS_VERSION` 10 → 11,
  with its comment). Serial with T247 (shipped-defaults adjacency). Depends on T269.
- [x] T271 [US14] Wire the hint into the three plain-click paths (additive — the click is never
  prevented): `packages/ui/src/renderer/terminal/terminal-link-activation.ts`,
  `packages/ui/src/renderer/editor/use-editor.ts`,
  `packages/ui/src/renderer/preview/providers/markdown/markdown-body.tsx` (its mousedown/click handlers,
  `:601-653`; S6); RED cases first in `packages/ui/tests/unit/terminal-link-activation.test.ts`,
  `packages/ui/tests/component/editor-link-gestures.test.ts` and `packages/ui/tests/component/preview-links.test.ts`
  (a plain click shows the hint **and** keeps its caret/selection/mouse-report meaning). Depends on T269,
  T263.
- [x] T272 [P] [US16] RED: create `packages/ui/tests/component/link-target-readout.test.tsx` — terminal
  and editor status bars show the hovered link's full target per FR-167a (the first reading: absolute
  where the panel has a base, as written where it has none; OSC 8: the declared target, not the text)
  at the bottom-left after persistent content, clear on leave, and render nothing while the bar is
  hidden; `preview-link-readout.test.ts` and `preview-status-bar.test.ts` stay green unchanged.
- [x] T273 [US16] GREEN for T272: extract `packages/ui/src/renderer/common/link-target-readout.tsx` from
  `packages/ui/src/renderer/preview/preview-status-bar.tsx` (output unchanged) and use it in
  `packages/ui/src/renderer/editor/status-strip.tsx` and `packages/ui/src/renderer/terminal/terminal-status-bar.tsx`,
  fed from `terminal/hovered-link.ts` and the editor's hovered-link state. Depends on T263.
- [x] T274 [P] [US15] RED: rewrite `packages/core/tests/unit/link-menu.test.ts` for `buildLinkMenu`
  (data-model §16.7): FR-170's order and offered rules for every SC-025 fixture (in-project file, with a
  provider, disabled provider, executable → Open Program not Default Program, out-of-project file,
  folder, web, protocol, anchor, unresolved — **FR-170a**: the unresolved fixture draws its applicable
  target items disabled, decided from `applicable`, data-model §16.12; and a **pending** fixture —
  `resolution: 'pending'`, before main answers — emitting FR-170c's renderer-known rows with rows 5/6
  absent, so no surface filters rows itself, data-model §16.21), label **Copy Link to Clipboard**,
  id `copyLink`; Open Link
  invokes the context's `openLink` (the surface's own Ctrl+click action, data-model §16.11); update
  `packages/core/tests/unit/menu-sections.test.ts` and `packages/ui/tests/unit/menu-sections.test.ts`.
- [x] T275 [US15] GREEN for T274: `packages/core/src/links/menu.ts` (`buildLinkMenu`, absorbing
  `fileLinkMenuItems` and `packages/core/src/links/targets.ts`'s `LINK_TARGETS` / `linkTargetStates`, with
  S8's executable row); core index exports for the new builder. **Deletes nothing** that a renderer file
  still imports — the old exports stay until T277 has moved every caller. Depends on T252.
- [x] T276 [P] [US15] RED: component tests for the opening rule and identity — rewrite
  `packages/ui/tests/component/terminal-file-link-menu.test.ts`, `editor-content-menu.test.ts`,
  `editor-web-link-menu.test.ts`, `preview-link-menu.test.ts`, `link-out-of-project.test.ts`,
  `link-target-actions.test.ts` and `link-copy-address.test.ts`: right-click / `menu.open` over a link with
  no selection opens the Link menu **instead of** the panel's menu, with a selection the panel's menu,
  away from a link the panel's menu unchanged; the three surfaces' Link menus are equal for each fixture
  (SC-025 — items, order, labels, enablement; the Open Link chord only where bound); test id
  `menu-item-Copy Link to Clipboard`; one bounded resolution per opening; an unresolved link's applicable
  target items drawn disabled (FR-170a); **FR-170b / FR-170c** — the menu opens at once with only the
  renderer-known rows (rows 5/6 not drawn; Open In ▸ not drawn for a folder by grammar — a trailing
  separator or the project root itself, FR-168b; no Open in
  OS Explorer for web/protocol), and when the (fake, delayed) answer arrives it adds, enables or removes
  rows — a folder removes rows 2, 3, 5, 6 — while a late answer after close changes nothing; before the
  answer Open Preview is drawn only when the renderer's provider registry accepts the extension (an
  in-project `.ts` shows none, even for a moment); **FR-158c / FR-158d** — a folder by grammar opens its menu
  with **no** resolution request; a link in the project by name that resolves **outside** it (R35, FR-168a)
  has rows 2 and 3 **removed** by the answer, not left disabled; **the preview** (plan *Corrections,
  eleventh pass*) — in `preview-link-menu.test.ts`, one case per SC-025 fixture: 044's `file` and
  `outside` kinds send the menu-open `throng:links:resolve` and fill rows 2 – 6 from its answer, `heading`
  maps to the anchor rows, `external` with an `http(s)` URL to web, `external` with `mailto:` to protocol,
  and `inert` (including `tel:` / `slack:` in a preview) opens no Link menu; and
  `packages/ui/tests/component/link-executable-refusal.test.ts` (`:243-257`): an executable runs from
  **Open Program**, and Open in OS Default Program is not offered for it (S8).
- [x] T277 [US15] GREEN for T276: create `packages/ui/src/renderer/links/link-menu.ts` (items →
  `MenuAction[]`, Open In ▸ via `submenu` and `packages/ui/src/renderer/editor/open-in-targets.ts`; each
  surface passes its own Ctrl+click as `openLink` — the preview's is 044's follow, so Open Link and the
  gesture never diverge; a component case in `preview-link-menu.test.ts` pins it) and
  `packages/ui/src/renderer/links/open-link-or-panel-menu.ts`; remove the link runs from
  `packages/ui/src/renderer/terminal/terminal-content-menu.ts`, `terminal-panel.tsx` (`:380`),
  `packages/ui/src/renderer/editor/content-menu.ts`, `packages/ui/src/renderer/preview/content-menu.ts`;
  fold `packages/ui/src/renderer/links/link-menu-items.ts` into it and delete what is left; **then**
  delete `packages/core/src/terminal/link-menu.ts` (with `packages/core/tests/unit/terminal-link-menu.test.ts`),
  `fileLinkMenuItems` and `packages/core/src/links/targets.ts` (with `packages/core/tests/unit/link-targets.test.ts`),
  moving their still-true cases to `link-menu.test.ts`; update the expected module list in
  `packages/core/tests/unit/links-no-os-names.test.ts:151-165` (out: `targets.ts`, and `classify.ts` once
  T290 has emptied it; in: `resource-class.ts`, `sanitise.ts`, `protocol-uri.ts`, `refused-schemes.ts`,
  `known-extensions.ts`); and update the callers in
  ...
  `packages/ui/tests/unit/redraw-menu-parity.test.ts:81-82`. FR-170b / FR-170c: the open menu re-renders
  when main answers — `packages/ui/src/renderer/context-menu-provider.tsx` exposes an update of the open
  menu's items and `packages/ui/src/renderer/workspace/context-menu.tsx` re-renders them; a late answer
  after close is dropped. Depends on T275, T256.
- [x] T278 [US15] Relabel **Copy Link Address → Copy Link to Clipboard** everywhere outside `specs/` and
  the constitution's history block: every file `git grep -n "Copy Link Address\|copyLinkAddress" -- packages README.md docs CONTRIBUTING.md .claude`
  lists — code comments, ids, test ids, `docs/quick-start.md` (`:262`, `:266`, `:440`, `:485`),
  `CHANGELOG.md` (unreleased entries only) and `.claude/agents/throng-renderer-ui.md:44`. The grep must
  return nothing afterwards; separately, the `## Unreleased` section of `CHANGELOG.md` (lines 30 – 102 today) must hold no
  "Copy Link Address" (released sections keep their history); and add one line under the constitution's
  SYNC IMPACT REPORT (`.specify/memory/constitution.md:22-26`) recording that the three sites it lists as
  "not yet matching the label" now match — a record update, not an amendment. Depends on T277.

**Checkpoint**: SC-023, SC-025, SC-027 green at component layer.

---

## Phase 22: Round four — docs, E2E cases, budget and gate

- [x] T279 [P] Docs in the same commits as the behaviour — **each GREEN task above carries the doc lines
  for the behaviour it changes in its own commit** (FR-090); T279 is the closing pass that checks the
  whole set reads as one and fills any gap: `README.md` and `docs/quick-start.md` —
  syntactic links (drawn without checking), the five classes and what each Ctrl+click does, the
  protocol allowlist and the always-refused schemes, spaces in paths (the maintainer's rule and the known
  extensions setting with added/removed), the plain-click hint, the hand pointer, the status-bar target,
  the Link menu; `CHANGELOG.md`'s unreleased 045 entry. Satisfies FR-090 (amended round four).
- [x] T280 [US10] [US14] E2E **cases inside existing declarations only**, in
  `packages/ui/tests/e2e/terminal-link-once.e2e.ts`: in *"a PLAIN click on a link opens nothing"* (`:520`)
  assert the hint appears over real cell geometry, fully on screen, the hand pointer shows without Ctrl,
  and nothing opens — **and** (D5's never-quiet half, which this declaration runs on the hosted gate
  because it has no `skipIfElevated()`) print the link then keep repainting a status line in place,
  asserting the link is marked at rest within the throttle interval while output never pauses; in
  *"Ctrl+clicking a link on the ALTERNATE screen"* (`:560`), **after** its exactly-once assertion, have
  the `altlink.cjs` fixture repaint and then leave the alternate screen (`ESC[?1049l`) and assert no mark
  survives the switch (D5's buffer-switch half). That declaration calls `skipIfElevated()`, so the hosted
  gate skips it (CLAUDE.md): at the gate the buffer-switch half is held by T233 (unit) and quickstart §10
  row 9, and it runs on every unelevated local E2E run. The OSC 8 halves through `osc8HalfRuns`.
  One-sentence reason (Principle V): only a real xterm renderer has cell geometry, a real pointer and a
  real alternate-screen switch. **No declaration added.**
- [x] T281 Re-read `packages/ui/tests/e2e/e2e-budget.json`: `"total": 570`, `@terminal` 107, unchanged;
  run the budget, tag and tier-plan guards (`e2e-budget.test.ts`, `e2e-tags.test.ts`,
  `tier-plan.test.ts`). Depends on T280, T292.
- [ ] T282 [P] File the round's tracked gap through the `github-issues` skill: protocol handlers are not
  de-elevated (FR-038 names two targets; plan *Reported to the maintainer* item 10) — enhancement,
  `area:terminal`, milestone per the repo's template. **The coordinator does this, not the spec.**
- [x] T295 *(added after the ninth analysis)* Hands-on — re-measure SC-004 per
  [quickstart.md](./quickstart.md) §5 (streaming throughput with link detection on and off) after T237's
  view pass lands, and record the figures, commit and machine beside the `3cb9c18c` measurement, as T114
  did. A result over SC-004's 5% ceiling becomes an appended task with its own RED test, never a silent
  retune of `LINK_MARK_THROTTLE_MS`. Depends on T237, T262.
- [ ] T283 Hands-on, maintainer — [quickstart.md](./quickstart.md) §10 rows 1 – 13, including Claude
  Code full-screen (row 9).
- [x] T284 Dispatch `npm run gate` against this branch on a hosted runner (`gh workflow run gate.yml
  --ref feature/S045-I198-I394-terminal-and-file-links`), watch it once, take the verdict from
  `gh run view`, and quote the **run URL and SHA**. Supersedes T172 as the done-ness gate for round four.
  Depends on every task in Phases 17 – 22.

**Parallel (round four)**: the RED tasks T235, T238, T240, T244, T246, T250, T251, T253, T255, T257,
T259, T261, T266, T268, T272, T274, T276, T285, T287, T289, T291, T293 may be written together, **except** these pairs,
which share a test file and are written in sequence: T238 → T285 (`terminal-link-affordance.test.ts`),
T248 → T287 (`link-setting-live.test.ts`), T233 → T285 (`terminal-link-view-marks.test.ts`), and
T289 → T271 (`terminal-link-activation.test.ts`), T291 → T285 (`link-detection-switches.test.ts`),
T294 → T276 / T277 for `link-out-of-project.test.ts` (T294 edits its three `followLink` calls first; T276
rewrites the file after), T287 → T294 for `link-setting-live.test.ts` (both Phase 19/20, in phase order),
T261 → T294 (`link-parity.test.ts`), T294 → T277 (`link-default-action-wiring.test.ts`), T294 → T276
(`link-executable-refusal.test.ts`),
T291 → T263 (`editor-web-links.test.ts`), T293 → T263 (`link-position-open.test.ts`), T255 → T293 (`file-link-resolver.integration.test.ts`) and T285 → T267
(`terminal-hovered-link.test.ts`) — each
earlier-phase test edit first, so no phase waits on a later one.
Serialise GREEN tasks that share a file: `packages/core/src/index.ts` (T236, T241, T252, T256, T264, T265,
T275); `packages/core/src/links/limits.ts` (T236, T256, T264, T265);
`settings-metadata.ts`/`app-settings.ts` (T247) before `shipped-defaults.ts` (T270);
`use-terminal.ts` (T237, T249, T265, T286, T290); `terminal-link-activation.ts` (T265, T271, T286, T290, T294);
`link-marks.ts` (T239, T265, T290); `hovered-link.ts` (T267, T273, T286, T294); `terminal-panel.tsx`
(T249, T277); `scan-line.ts` (T245, T252); `external-url.ts` / `preload.cts` / `global.d.ts` (T258, T288, T294); `use-editor.ts` / `link-decorations.ts` (T239, T249, T263, T265, T267, T271, T294);
`file-link-resolver.ts` / `link-ipc.ts` (T256, T260, T294); `link-actions.ts` (T258, T260, T290, T294); `terminal.css` (T239 only).
**Never run two test commands at once on this machine.**

**Tests these round-four supersessions permit to change**: `packages/core/tests/unit/link-resolve.test.ts:170-173`
and `:367` (FR-176); the extended-reading cases in `link-detect.test.ts` (FR-173/FR-179); the
existence-based marking cases named in T263; the held-reply cases in `terminal-file-link-provider.test.ts`
(FR-155); the FR-154 cases in `terminal-link-affordance.test.ts` that expected an unreachable `file:`
OSC 8 target to look like text, and the hand-only-with-modifier cases (FR-163, FR-164); every "Copy Link
Address" assertion (FR-175); `terminal-link-idle-scan.test.ts`,
`link-cache.test.ts` and `terminal-link-menu.test.ts` (deleted, their true cases moved); and the rest of
spec.md's *Added 2026-09-19 while planning* list — `link-guards.test.ts` (TTL and idle edges),
`editor-content-menu.test.ts`, `link-out-of-project.test.ts`, `link-target-actions.test.ts`,
`link-copy-address.test.ts`, both `menu-sections.test.ts`, `link-menu.test.ts`, `preview-links.test.ts`,
`terminal-link-activation.test.ts`, `link-parity.test.ts`, `link-setting-live.test.ts`,
`link-failure-notice.test.ts`, and additions to the settings, theme and shipped-defaults tests; and
`link-classify.test.ts:26-30` (T289), `terminal-hovered-link.test.ts:145`, `:172` (FR-168),
`editor-web-links.test.ts:116` and its channel assertion (FR-159), `link-detection-switches.test.ts`'s
channel (FR-159, FR-060a) and a named exemption in `links-no-os-names.test.ts` (FR-178b).
`link-web-url.test.ts` must not change (FR-159a). Also `file-link-resolver-network.test.ts`,
`link-hover-text.test.ts`, `link-detect-spaces.test.ts` (header and markers only, T242),
`terminal-file-link-menu.test.ts`, `editor-web-link-menu.test.ts`, `preview-link-menu.test.ts`,
`link-executable-refusal.test.ts:243-257` (S8), `link-de-elevated-open.integration.test.ts` (additions),
`terminal-hovered-link.test.ts:189-224`, `:240-246` and `link-detection-switches.test.ts:182` (FR-163);
`link-ipc-confinement.integration.test.ts:197-207` and `file-link-resolver.integration.test.ts:310-327`
(FR-158b, S10); `link-default-action-wiring.test.ts:231`, `menu-icon-tokens.test.ts:97-98`, `:297`,
`:598-599`, `redraw-menu-parity.test.ts:81-82` (FR-169) and `link-targets.test.ts` (deleted, S8);
`terminal-links.e2e.ts` (`:12-40`, `:66`, `:84-92`, `:153-156`, `:213-307`, `:313`, `:370-452`; T292) and
`links-no-os-names.test.ts:151-165`'s module list (T277, T290); `link-actions-router.test.ts` and
`link-position-open.test.ts` (T293, the awaited follow); `terminal-link-base-directory.test.ts:108-127`
(its hover-ask assertion moves to the follow request, T293) and its component twin (import only, T265);
`link-out-of-project.test.ts`, `link-setting-live.test.ts`, `link-parity.test.ts:210` (`followLink`'s
signature, T294); `link-ipc.contract.test.ts` (the `follow` channel and its titles, fakes and T147 block; T293); `link-default-action-wiring.test.ts` (`:161`,
`:184`, `:236`, `:319`, `:348`, `:357`) and `link-executable-refusal.test.ts` (`:124`, `:145`, `:162`)
(T294, `followLink`'s signature); additions only to `terminal-link-once.e2e.ts`
(T280), `electron-shell-integration.test.ts` and `shell-integration-contract.ts` (T259). The two lists
are the same set.
`packages/ui/tests/unit/external-url.test.ts` and `packages/ui/tests/e2e/terminal-modified-enter.e2e.ts:233`
**must still not change**.

---

## Phase 23: Round five — one hover, switches that mean all links, one extension list, D6, a subtler underline, D7, D8 (FR-169a, FR-169b, FR-180 – FR-187, D6 – D8, SC-028 – SC-034)

**Goal**: the maintainer's six items from 2026-09-20, every one additive to the spec and none of them
touching the maintainer-verified space table. **Independent test**: quickstart §10 rows 1, 2, 7, 12 plus
the three prompts and the Claude Code header with a spaced working directory.

- [x] T296 [US6] [US13] The three descriptor changes and one retirement, RED first in
  `packages/core/tests/unit/settings-metadata-links.test.ts`, `app-settings.links.test.ts`,
  `app-settings.terminals.test.ts`, `bounds-guard.test.ts` and `editor-settings.test.ts`: the timeout
  keeps its **key** and gains the label **Link resolution timeout** with a description that says
  "resolve", never "exists" (FR-181); both switches lose the word *file* from their labels and say every
  kind is governed (FR-180b); and `terminals.linkHoverDelayMs` is **retired** — leaf, default, parse
  line, clone field, descriptor and its `parseAppSettings` clamp — with a persisted value dropped on read
  and absent after the next write (FR-169b, 019 FR-023). GREEN in
  `packages/core/src/config/app-settings.ts` and `settings-metadata.ts` (serial, both files); the
  completeness guard passes unchanged. Satisfies FR-169b, FR-180b, FR-181, SC-030.
- [x] T297 [US13] Invert the known-extension setting to **one** list: RED for the single leaf
  `editor.links.knownFileExtensions` defaulting to the shipped `KNOWN_FILE_EXTENSIONS`, for its short
  description, and for the **migration** — a document carrying `{ added, removed }` reads as the set
  those edits resolved to, the old keys are gone after the next write, and parse → serialise → parse is a
  fixed point; plus the inverted empty value (`[]` = no extension, never "unset"). GREEN:
  `knownFileExtensionsSetting` in `app-settings.ts`, the one descriptor in `settings-metadata.ts` (which
  stops importing `KNOWN_FILE_EXTENSIONS`), `knownFileExtensionsSet` in
  `packages/core/src/links/known-extensions.ts` — the twin of `protocolAllowlistSet` — and
  `packages/ui/src/renderer/links/link-scan-options.ts` reading through it, with `linkScanKey` down to
  two members. `resolveKnownExtensions` survives for the migration alone. Satisfies FR-182, FR-182a,
  FR-182d, SC-030.
- [x] T298 [P] [US6] RED for the switches covering every kind:
  `packages/ui/tests/component/link-detection-switches.test.ts` (with a switch off, **no** kind is
  marked, hovered, hinted, followed or given a Link menu — detected path, web, allowlisted protocol and
  OSC 8 alike), `editor-web-links.test.ts` (its "web links still work with the switch off" case inverts)
  and `link-setting-live.test.ts` (the change lands with no remount, and repaints a terminal at an idle
  prompt). Satisfies FR-180, SC-029.
- [x] T299 [US6] GREEN for T298 — the four seams, each returning nothing rather than filtering after the
  fact: `linkHitsBetween` (`editor/link-decorations.ts`), `linksOnLine`
  (`terminal/file-link-provider.ts`), `terminalViewScan` (`terminal/link-view-marks.ts`, via a shared
  `NOTHING_SCANNED`) and the three OSC 8 closures in `terminal/use-terminal.ts` (`setHoveredUri`,
  `openTerminalLink`, `oscLinksInView`) behind one `linksEnabled()` reader. Satisfies FR-180, FR-180a.
- [x] T300 [P] [US14] RED for the hover title: `packages/ui/tests/unit/terminal-link-affordance.test.ts`
  (the hovered link's `title` on the panel host, the caller's text winning over the fallback, cleared on
  leave and on dispose, none for a dead OSC 8 target), `packages/ui/tests/component/link-decorations.test.ts`
  (the editor's title), `preview-links.test.ts` / `preview-sanitise.test.ts` (the preview's title is the
  target alone, and its readout is unchanged), `packages/ui/tests/unit/link-parity.test.ts` and
  `link-wording-parity.test.ts` (the three surfaces agree), and
  `packages/ui/tests/unit/floating-surfaces.test.ts` (the `.terminal-link-tip` entry goes with its
  element). Satisfies FR-169a, SC-028.
- [x] T301 [US14] GREEN for T300: `terminal/link-marks.ts` (the host gains `title`, `sync` gains
  `hoverTitle`), `terminal/use-terminal.ts` (one `hoveredTitle`, fed by the same
  `hoveredLinkReadoutText` the status-bar readout uses — FR-169a(ii)), `editor/link-decorations.ts`
  (`markFor` titles by `linkFirstReadingByName`, a web link by its uri) and
  `preview/providers/markdown/sanitise.ts` (the `title` is `displayTarget`, with no gesture wording).
  **Delete** the `.terminal-link-tip` element, its CSS block, `LINK_TIP_LEAVE_GRACE_MS`, both tip timers
  and the delay reader. `linkHoverText`, `previewLinkHoverText` and `uriHoverDestination` stay, each with
  one caller — the hint (FR-169a(i)). Satisfies FR-169a, SC-028.
- [x] T302 [P] [US1] [US12] **RED for D6, before any production change** (the replicating-bugs gate):
  create `packages/core/tests/unit/link-detect-known-directory.test.ts` — with no predicate the answers
  are exactly the case table's; the **longest** landing wins, so a two-word folder is not cut at its
  first word; a terminator still wins outright; a sibling sharing a string prefix does not match; the
  scan is still capped by `MAX_PATH_SPACE_WORDS` — and
  `packages/ui/tests/unit/terminal-link-prompt-cwd.test.ts`, driving the real provider with a working
  directory of `D:\git\throng_tests\test 1`: the PowerShell, cmd and Git Bash prompts, the Claude Code
  CLI header, an ancestor extending, prose not extending, and **with no working directory known,
  behaviour exactly as before**. `packages/core/tests/unit/link-detect-spaces.test.ts` MUST NOT be
  touched. Satisfies D6, FR-183, SC-031.
- [x] T303 [US1] [US12] GREEN for T302: `DetectOptions.namesKnownDirectory` and the `directoryEnd`
  landing in `scanAcrossSpaces` (`packages/core/src/links/detect.ts`), the same option on `ScanOptions`
  (`scan-line.ts`), `namesTerminalDirectory` in
  `packages/ui/src/renderer/terminal/terminal-link-activation.ts` (folding both sides through
  `absolutePathByName`, memoised on the cwd, equality-or-ancestor at a separator boundary), and the
  threading from `cwd-store.ts` through `terminalLinkBaseDirectory` into `file-link-provider.ts` and
  `link-view-marks.ts`. No filesystem access is added anywhere (SC-021 unchanged). Satisfies FR-183,
  FR-183a – FR-183f.
- [x] T304 [US10] The subtler underline (FR-184): `terminal/terminal.css` and the editor's `linkTheme`
  mix `linkUnderline` at 45% against transparent at rest and stay solid on hover; the Markdown preview,
  which may use neither that token nor a colour function (`preview-css-tokens.test.ts`, SC-005), uses
  `border` at rest and `syntaxFunction` on hover. **No new theme token.** Satisfies FR-184, FR-184a,
  SC-032.
- [x] T305 `SHIPPED_DEFAULTS_VERSION` stays **11**: the bump to 12 written in T296 is reverted and the
  reason recorded beside the constant in `packages/core/src/config/shipped-defaults.ts` — two settings,
  no theme token, and a settings change reaches an install through the tolerant per-field parse with no
  version gate. `shipped-defaults-upgrade.test.ts` keeps its version-11 cases. Satisfies FR-184b.
- [x] T306 The resolver reads the list through the accessor: `packages/ui/src/main/file-link-resolver.ts`
  drops its now-dangling `KNOWN_FILE_EXTENSIONS` / `resolveKnownExtensions` imports and calls
  `knownFileExtensionsSet` on the setting. Without it every follow threw and answered `failed`, so the
  surfaces and main disagreed about where a span ended — FR-178a's H1 trap, in its new shape. Depends on
  T297. Satisfies FR-182b.
- [x] T307 [US10] The deleted tip was two E2E specs' "a link resolved" signal. In
  `packages/ui/tests/e2e/terminal-link-once.e2e.ts` and `terminal-links.e2e.ts`, wait on the status-bar
  readout and assert the panel's `title`, with the three-phase LEAVE / ENTER / READ probe kept and
  re-justified by xterm's per-line provider cache and the main-process round trip rather than by a tip
  delay. **Cases inside existing declarations only**; no declaration and no budget entry moves.
  Satisfies FR-169a, SC-028.
- [x] T308 [US10] [US14] One **added** E2E declaration in `packages/ui/tests/e2e/terminal-links.e2e.ts`
  for what only a real window shows — a real hover over real cell geometry putting the target in the
  native `title`, and the at-rest underline against its hovered one — and raise
  `packages/ui/tests/e2e/e2e-budget.json` by one in the **same commit** (the ratchet fails both ways).
  Tags: one significance tag and one category tag (`@terminal`), or `e2e-tags.test.ts` fails the build;
  add it to `parallel-plan.json` only if it neither drives a menu, a preferences window nor a
  long-running real shell. *Owned by the sibling agent working alongside this amendment.*
- [x] T309 **Docs currency — a constitutional gate, not a nicety.** Round five renames
  user-facing settings and changes what a switch does, and the docs described round four until
  `9a13eb87` and `2f966318` rewrote them: `docs/quick-start.md` and `CHANGELOG.md`'s **Unreleased** 045
  entry now say what round five ships — one hover showing the target, switches that govern every kind,
  **Link resolution timeout**, one editable extension list with the cost of storing it whole, spaced
  prompt folders, the subtler underline, and a bare email address being a `mailto:` link (FR-185, the
  user-visible half of D7). `9a13eb87` also corrected `known-extensions.ts`'s section header, which
  still claimed the setting stored edits.
  `README.md`'s settings paragraph was the last thing outstanding — it documented
  `editor.links.knownFileExtensions.added` and `.removed`, a setting shape that no longer exists
  (FR-182) — and it now names the one list, the widened switches and the bare address. Satisfies
  FR-090.
- [ ] T310 Hands-on, maintainer — the six round-five items on a real desktop: hover in a terminal, an
  editor and a preview (one title, the full target, nothing else); each switch off and on; the three
  prompts and the Claude Code header in a folder whose name contains a space; the extension list edited
  live; the underline at rest against hover in a light and a dark theme.
- [x] T312 [US8] [US13] **D7 — a bare email address is a `mailto:` link, not a file.** RED first and
  shown to the maintainer before any production change (the replicating-bugs gate): create
  `packages/core/tests/unit/link-bare-email.test.ts` with six cases — **four red** (the address is
  claimed as a path candidate; it is a `mailto:` link as the preview already draws it; in a sentence the
  span is the address and not the punctuation after it; with `mailto` **off** the allowlist it is no
  link **and still no path**) and **two green controls** (an explicit `mailto:` stays one span; a path
  carrying an `@` stays a path). GREEN: `detectBareEmailSpans` and `bareEmailsAreLinks` in
  `packages/core/src/links/protocol-uri.ts`, wired in `scan-line.ts` so the scan **claims** every
  address and **publishes** only the allowed ones (FR-186 — the gated first attempt passed four cases
  and failed the sixth). The span's `uri` is `mailto:` plus its text, the first span here where the two
  differ (FR-185a; data-model §17.7). Both panel types are fixed by the one scanner (FR-104); the
  Markdown preview was already right, by markdown-it's autolinking. Satisfies FR-185, FR-186, D7,
  SC-033. Committed as `3e8be8c1`.
- [x] T313 [US16] **D8 — the readout and the title name where the click goes.** RED first and shown to
  the maintainer before any production change: create `packages/ui/tests/unit/link-readout-truth.test.ts`
  with six cases — `/tmp` and `/etc/hosts` read out as written rather than under the project; **a rooted
  path that really does resolve inside the project also reads as written** (the case that proves the
  rule rather than the symptom); and three controls that must not move — a drive form reads as its
  drive, a relative path against its base directory, an absolute path as itself. GREEN: step 3 of
  `linkFirstReadingByName` (`packages/ui/src/renderer/links/path-by-name.ts`) returns the text, which is
  what that module's own header already said for its undecidable cases. **Rewrite, do not delete**, the
  two cases that pinned the superseded rule —
  `packages/ui/tests/unit/link-by-name-readings.test.ts` ("a rooted path that is NOT a drive form reads
  against the project root first (FR-024)") and
  `packages/ui/tests/component/link-target-readout.test.ts` ("shows a rooted non-drive path against the
  project root (FR-024)") — each asserting as-written and carrying a note naming what replaced it.
  FR-024 is **not** edited: it governs resolution. Satisfies FR-187, D8, SC-034. Committed as
  `c43d895f`. **Known gap left open on purpose**: the plain-click hint reads the same paths through
  `clickTargetByName`'s relative branch and promises an editor — reported to the maintainer, whose
  wording decision it is (FR-187's known gap; plan *Reported to the maintainer — round five* item 6).
- [x] T311 Dispatch `npm run gate` against this branch on a hosted runner
  (`gh workflow run gate.yml --ref feature/S045-I198-I394-terminal-and-file-links`), watch it once, take
  the verdict from `gh run view`, and quote the **run URL and SHA**. Supersedes T284 as the done-ness
  gate for round five. Depends on every task in Phase 23.
  **Green at `e771fe2a`**, all eight stages —
  https://github.com/Bidthedog/throng/actions/runs/35523385671 — lint 24.7s, typecheck 15.7s, build
  4.1s, unit 5,350, component 2,399, integration 1,093 (12 skipped), contract 306, e2e 523 across
  both tiers (175 parallel / 348 serial) in 27m32. Zero failed, zero flaky.
  Two earlier runs are part of the record rather than noise: `9a13eb87`
  (https://github.com/Bidthedog/throng/actions/runs/35518491102) went green over the six original
  round-five items before D7 and D8 were reported, and the run on `717f2efc` was CANCELLED once D8's
  fix superseded it rather than spending a runner on a commit that was already two behind.

**Parallel (round five)**: T296, T297 and T302 may be written together (different files). T312 arrived
after the rest and touches `protocol-uri.ts` / `scan-line.ts`, which nothing else in this phase does, so
it serialises against nothing. Serialise
`app-settings.ts` / `settings-metadata.ts` (T296 → T297), `use-terminal.ts` (T299 → T301),
`link-decorations.ts` (T299 → T300/T301), `detect.ts` / `scan-line.ts` (T303 alone) and
`terminal.css` (T304 alone). T306 follows T297. T307 and T308 both touch `terminal-links.e2e.ts` and are
written in sequence, T307 first. **Never run two test commands at once on this machine.**

**Tests these round-five supersessions permit to change**: `floating-surfaces.test.ts` (the
`.terminal-link-tip` entry only), `terminal-link-affordance.test.ts`, `link-parity.test.ts`,
`link-wording-parity.test.ts`, `link-decorations.test.ts`, `preview-links.test.ts`,
`preview-sanitise.test.ts`, `preview-link-readout.test.ts`, `preview-images.test.ts`,
`link-detection-switches.test.ts`, `link-setting-live.test.ts`, `editor-web-links.test.ts`,
`app-settings.links.test.ts`, `app-settings.terminals.test.ts`, `settings-metadata-links.test.ts`,
`bounds-guard.test.ts`, `editor-settings.test.ts`, `notification-settings.contract.test.ts`,
`command-recipe.test.ts`, `terminal-flavour-reports-directory.test.ts`, and the two E2E signal sites
(T307). **Additions only**: `link-detect-known-directory.test.ts`, `terminal-link-prompt-cwd.test.ts`,
`link-bare-email.test.ts` (T312), `link-readout-truth.test.ts` (T313) and T308's declaration.
**T313 changes exactly two existing cases**, both **rewritten and annotated, never deleted** —
`link-by-name-readings.test.ts`'s and `link-target-readout.test.ts`'s project-root rows (FR-187b);
every other case in both files, the drive-form, relative and OSC 8 rows included, stays as it is.
**`packages/core/tests/unit/link-detect-spaces.test.ts`
MUST NOT change** — it is the maintainer-verified table, and FR-183 is shaped precisely so that it does
not (SC-031). **No existing case changes for T312** either: the address grammar takes a range the path
grammar used to claim, and its two green controls are there to prove the paths that still hold paths
still pass.

**Checkpoint**: SC-028 – SC-034 green at every layer the gate runs. T309 and T311 done — the hosted
gate is green at `e771fe2a` (unit 5,350, component 2,399, integration 1,093, contract 306, e2e 523,
zero flaky). **T310, the maintainer's hands-on pass, is the only task left in this phase**, and
FR-187's hint gap is open by decision rather than by omission.
