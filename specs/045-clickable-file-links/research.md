...
|---|---|---|
| **O1** | Is `provideLinks` called only on hover, or also during viewport render? (R2) | the FR-072 unit test with a fake `Terminal`, then `npx playwright test packages/ui/tests/e2e/terminal-links.e2e.ts` |
| **O2** | Is `IDeElevator.isAvailable()` true in **UI main**? (R7) | `npx vitest run --project contract packages/platform-windows/tests/contract/windows-de-elevated-launcher.contract.test.ts` and an `@admin` case |
| **O3** | Does `explorer.exe /select,<path>` behave identically to `shell.showItemInFolder` for a UNC path and for a path with a comma? (R7) | a new `@admin` integration case in `platform-windows/tests/integration/` |
| **O4** | Does Claude Code actually emit OSC 8 under `FORCE_HYPERLINK=1` in a throng terminal, on this Windows build? (US7 scenario 6) | the hands-on step in [quickstart.md](./quickstart.md) §6 — the only thing that can answer it |
| **O5** | Does widening `preview.followLink`'s scope leave `keybindings-collision.test.ts` green? (R9) | `npx vitest run --project unit packages/core/tests/unit/keybindings-*.test.ts` |
| **O6** | Measured cost of the FR-073 visible-range scan on the largest fixture document, for SC-004's editor half | the annotation in quickstart §5 |
