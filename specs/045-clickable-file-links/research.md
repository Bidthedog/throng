...
*R10(a) superseded 2026-09-20 (round five, maintainer M12; spec FR-169a, FR-169b, R38): there is no
tooltip of throng's to word and no delay to share — the hover is a native `title` carrying the target,
and the delay setting is retired. The wording this entry derives lives on in the link hint, where its
reasoning holds unchanged.*
...
| **O8** *(2026-09-18)* | Does D1's reproduction fail as recorded, and does it match what the maintainer saw? (R18) | ~~**OPEN.** T135: run `core/tests/unit/link-resolve-unc.test.ts`, show the output, wait for the maintainer.~~ **SETTLED 2026-09-18: yes.** The maintainer confirmed the reproduction matches what they saw; R18's reading was right, and the fix is contract R12 (`join` keeps both leading separators of a UNC base; `..` stops at the server and share), committed with the adopted repro in `e1092d4c`. The provider-qualified half is FR-003g (`5f26c58a`). |
...
## Round four *(2026-09-19)* — R25 – R35

Input: spec *Session 2026-09-19 (round four)*; plan *Amendment 2026-09-19, round four*. Nothing above is
rewritten; where an entry above no longer holds, the entry here says so.

## R25. Rendering needs no disk, so the render-side resolution machinery goes

- **Decision**: `provideLinks`, the editor's decorations and the terminal's marks answer from
  `scanLinkLine` alone. The renderer's existence cache (`link-cache.ts`), the held reply
  (`file-link-provider.ts`, `LINK_ANSWER_DEADLINE_MS`) and the idle scan (`link-idle-scan.ts`,
  `LINK_IDLE_SCAN_MS`) are deleted.
- **Rationale**: FR-155. Each existed only to make a render-time existence answer cheap (R2, R19, R22);
  with no answer there is nothing to make cheap. Keeping them idle would be dead code (Principle VIII).
- **Alternatives**: keep the cache for click-time (rejected, R32); keep the idle scan with a shorter
  interval (rejected — the gate itself is D5's cause).
- **Supersedes**: R2's "hover-scoped ask", R19's renderer half, R22's idle scan, R24 (extended readings
  are gone).

## R26. One span per candidate — the maintainer's rule as the RED table encodes it

- **Decision**: `detectPathSpans(line, { knownExtensions })` per `link-detect-spaces.test.ts`'s header
  rules 1 – 5; FR-004's positioned/unpositioned pair remains the only multi-reading case and is still
  resolved in order by main at click time.
- **Rationale**: FR-173, FR-174, FR-179; the table is the maintainer's verified answer and wins edges.
- **Alternatives**: keep "longest existing reading" (withdrawn by FR-155).

## R27. The refused set's OS half is a curated port, not a registry query

- **Decision**: `IRefusedUriSchemes.refusedSchemes(): ReadonlySet<string>` in `platform-windows`
  returning a documented list (`ms-msdt`, `search-ms`, `search`, `ms-officecmd`, `ms-appinstaller`,
  `ms-cxh`, `ms-cxh-full`; the list's JSDoc cites the advisory behind each entry). The OS-neutral half
  (`javascript`, `data`, `vbscript`, `file`, `about`, `blob`) is a core constant.
- **Rationale**: Principle II; decidable, reviewable, contract-tested. A scheme's name does not say
  whether its handler executes a file, and a registry read per click is OS work on the click path.
- **Alternatives**: registry query (rejected, plan Complexity Tracking); denylist-only (rejected by the
  maintainer, M2 — allowlist first).

*R27 amended 2026-09-19 (R34, plan round four "Corrections after analysis"): `file` is not in the OS-neutral half classification reads; it is refused only at the external opener (FR-037).*

## R28. The known-extension setting stores edits; the allowlist stores a list

- **Decision**: `editor.links.knownFileExtensions.{added,removed}` (plan strand 9);
  `editor.links.protocolAllowlist` is a whole list.
- **Rationale**: settings materialise full values on disk; the only upgrade path for a changed list
  default (`planSettingsUpgrade`, `V4_EXCLUDE_GLOBS`) deliberately skips customised installs. For
  extensions that is the wrong trade — later extensions should reach everyone, and the maintainer's own
  words describe edits. For the allowlist it is the right trade — a release must never widen what a
  user's install hands the OS without the user choosing it.
- **Alternatives**: one list + frozen-copy upgrade per change (rejected — never reaches customised
  users, and every list change costs a version bump); a sign-prefixed single list (`+foo`, `-log`)
  (rejected — unreadable in the string-list editor, and still needs parsing rules the user must learn).

*R28 reversed 2026-09-20 (round five, maintainer M15; spec FR-182, FR-182c): the extension list is stored
**whole**, as the alternative above that this entry rejected. The rationale stands as analysis and was
simply outvoted by what the control has to look like — a delta the user cannot see is not "show a list of
the supported file extensions … and allow users to add and remove from them". The cost R28 names is real
and is now a recorded trade, not a solved problem: a later-shipped extension does not reach a user who
has edited their list. R36 carries the reasoning. The allowlist half of R28 is unchanged.*

## R29. "At most one hint in the whole app" across windows

- **Decision**: a per-renderer singleton store plus hide-on-`blur`.
- **Rationale**: a plain click in window B blurs window A before B's hint is shown, so A's hint is already
  gone; no cross-process message is needed.
- **Alternatives**: a main-process broadcast (rejected — IPC for a 2.5 s popover).

## R30. Wording a path's destination without the disk

- **Decision**: at hover, the destination is judged by name: FR-106's pure membership test against the
  project root (after the grammar's drive-form and relative readings) → editor or preview wording; else
  "show in OS Explorer". At click, the real resolution decides; if the two differ (an in-project name
  that does not exist), FR-160's notice is the answer.
- **Rationale**: FR-155 forbids a check at hover; FR-168 still wants the destination named.
- **Alternatives**: generic "Ctrl+Click to open" for every path (rejected — FR-168 names destinations).

## R31. A preview's in-document anchor in the Link menu

- **Decision**: Open Link and Copy Link to Clipboard only.
- **Rationale**: FR-169 wants one menu; an anchor is none of FR-157's classes, and absent-when-meaningless
  removes every target item. 044 FR-090 – FR-096 still decide what following it does (S6).

## R32. No click-time cache — T225 withdrawn

- **Decision**: every follow and every Link-menu opening resolves fresh, bounded by one per-request
  deadline. T225 (wire the watcher to `invalidateLinksUnder`) is withdrawn with the cache.
- **Rationale**: a deliberate act happens a few times a minute; a cache there saves microseconds and
  risks acting on a stale answer. M1 makes breakage throng's non-question at render, not at click.

## R33. Throttling the view pass, and why `onRender`

- **Decision**: subscribe to `term.onRender({start,end})` and `term.buffer.onBufferChange`; throttle at
  `LINK_MARK_THROTTLE_MS` with a guaranteed trailing run.
- **Rationale**: `onRender` fires when rows are drawn — the view, not the output — so it never reads data
  (FR-072) and fires for a repaint that writes nothing new (a status line redrawn in place). A trailing
  run guarantees the last drawn frame is marked (SC-021's "within one throttle interval").
- **Alternatives**: `onWriteParsed` with a debounce (rejected — a debounce is the quiet gate again).

## R34. Getting the platform's refused set into the renderer *(2026-09-19, after analysis)*

- **Decision**: `ipcMain.handle('throng:linkUri:refusedSchemes')` answering from `IRefusedUriSchemes`;
  the renderer asks once at startup and caches the answer for the window's life; drawing uses core's half
  until then. R27's list is amended: `file` is **not** in the OS-neutral half classification reads — it
  is refused at the external opener only (FR-037).
- **Rationale**: FR-159 says an allowlisted dangerous scheme is not a link, which is a drawing rule; the
  platform half cannot live in `@throng/core` (Principle II). The set is static per process, so one
  round trip suffices; main's click-time refusal covers the startup gap.
- **Alternatives**: duplicate the Windows list in the renderer (rejected — two lists to disagree); a
  synchronous preload value (rejected — `sendSync` blocks the renderer at startup).

## R35. Accepted trade in R30: a rooted path worded by name can be revealed at the click *(2026-09-19, after analysis)*

`/etc/hosts` with no `<root>\etc\hosts` is worded "open in throng …" at hover (its first reading is in the
project by name) and, under FR-160's first-existing-reading rule, revealed in OS Explorer at the click
when Git's mount table holds it. Accepted: the only alternative that words it correctly checks the disk
at hover, which FR-155 forbids; the click still does the right thing, and the mismatch is limited to
rooted non-drive paths in a project that lacks them. Recorded in FR-168's note.

*R35 widened 2026-09-19 (fourth analysis pass; spec FR-168a): the same trade covers a terminal relative
path whose working directory lies outside the project — worded by its first reading, followed by the
first reading that exists, which may be the project-root one. It is not limited to rooted paths.*

*R35 widened again 2026-09-19 (seventh analysis pass; spec FR-168b): a folder known **by grammar** — the
project root, or a path ending in a separator — is worded "show in OS Explorer" with no disk access. The
third accepted trade is an in-project folder written without a trailing separator: worded as an
in-project file, revealed at the click.*

*R32 amended 2026-09-19 (eleventh analysis pass): deleting the cache would have deleted FR-122's back-off, which was its lifetime; the back-off moves to main as `LINK_ROOT_BACKOFF_MS` (spec FR-122a).*

## R36. The extension list is stored whole, and what that costs *(2026-09-20, round five)*

- **Decision**: one leaf, `editor.links.knownFileExtensions: string[]`, shipping `KNOWN_FILE_EXTENSIONS`
  itself; the round-four `{ added, removed }` document shape is migrated on read and dropped on the next
  write (spec FR-182, FR-182a).
- **Rationale**: the maintainer asked for the supported extensions to *be* the options shown, added to
  and removed from. A delta cannot show them: the `array` control renders the value, and the value was
  two empty lists. Storing the list makes the control honest, which is the whole of the request.
- **Cost, accepted and recorded (FR-182c)**: an extension throng ships later never reaches a user who
  has edited theirs, because `planSettingsUpgrade` deliberately skips customised installs. If a later
  list ever matters, the remedy is a seeded upgrade under `SHIPPED_DEFAULTS_VERSION` for that release —
  not a return to the delta.
- **Trap it creates (FR-182d)**: the empty value inverted. `{ added: [], removed: [] }` meant "throng's
  list"; `[]` means "no extension at all". It is honoured as typed, never read as "unset", and it has
  already cost one silently-wrong fixture.
- **Alternatives**: keep the delta and render a computed preview beside it (rejected — two controls for
  one idea, and the user still cannot edit what they see); ship the list and diff it on write (rejected —
  that is the delta again, with the frozen-copy problem moved into the writer).

## R37. Crossing a space by what the app already knows *(2026-09-20, round five; D6)*

- **Decision**: detection accepts an optional, pure predicate — "does this text name the directory this
  surface is in?" — supplied by the renderer from 025's live working directory; a scan that has crossed a
  space may end on the **longest** text that satisfies it (spec FR-183).
- **Rationale**: the maintainer's prompt case (`D:\git\throng_tests\test 1>`) is unreachable from the
  text alone, and the 92-case table exists to stop the grammar guessing at it. Comparing with a value
  already held is neither a guess nor a filesystem question, so FR-155 is untouched and the table does
  not move — it is the difference between *asking* and *being told*.
- **Why the predicate is the caller's**: `@throng/core` may not map drive letters or mount forms
  (Principle II), and Git Bash prints `/d/git/…` while reporting `D:\git\…`, so the two spellings must
  meet in the renderer, through `absolutePathByName`.
- **Alternatives**: extend across a space while the next word has no extension (rejected — that is the
  path-likeness rule FR-173 replaced, and it re-opens every prose case in the table); ask the filesystem
  for the longest existing reading (rejected — FR-155, and it is round three's withdrawn design); special
  -case prompt lines by flavour (rejected — it would not fix the Claude Code CLI header, which is the
  same folder printed by something that is not a prompt).

## R38. One hover surface, and why it is the platform's *(2026-09-20, round five)*

- **Decision**: a native HTML `title` carrying the link's full target, on every surface; throng's own
  floating tip, its delay and its leave-grace are deleted (spec FR-169a, FR-169b).
- **Rationale**: the maintainer asked for exactly one thing on hover, and for it to be the link text in
  full. A native title is placed, timed, clamped and dismissed by the OS, which deletes four problems
  rather than re-solving them — viewport clamping, a flash while the pointer sweeps, re-arming when
  xterm re-evaluates its providers, and a configurable delay whose right value nobody knows. It also
  makes the title and the status-bar readout one string by construction (FR-169a(ii)).
- **What survives**: `linkHoverText`, `previewLinkHoverText` and `uriHoverDestination`, each with one
  caller — the plain-click hint, which the maintainer kept in as many words.
- **Cost**: the OS's delay and styling are not ours, and a `title` cannot be themed. That is the trade
  the request makes; nothing in FR-138 or FR-184 depends on it.

## R39. One reader per surface for the detection switch *(2026-09-20, round five)*

- **Decision**: each panel type's switch is read at the top of that surface's scan, plus one shared
  reader for the three OSC 8 closures; every seam returns an empty result rather than filtering
  afterwards (spec FR-180, FR-180a).
- **Rationale**: "all types of links" is only true if there is nowhere a kind can be produced from that
  the switch does not cover. Gating the *entry* of each producer makes a later kind's escape visible in
  review — it would have to be produced somewhere else entirely — where a filter over results would
  silently miss it.
- **Alternatives**: filter the scan's output by kind (rejected — the marks, the hover and the follow
  each consume a different product, so a filter has to be written three times); gate in the shared
  `scanLinkLine` (rejected — the scan is `@throng/core`'s and knows nothing about panels or settings).

## R40. A translucent underline without a new token *(2026-09-20, round five)*

- **Decision**: `color-mix(in srgb, var(--throng-colour-linkUnderline) 45%, transparent)` at rest in
  terminals and editors, solid `linkUnderlineHover` on hover; in the Markdown preview, `border` at rest
  and `syntaxFunction` on hover (spec FR-184, FR-184a).
- **Rationale**: a "resting" token would be a second name for a value derived from the first, and
  FR-138's parent-inheritance already makes `linkUnderline` the one place a user changes the colour.
  `color-mix` keeps that true at 45%.
- **Why the preview differs**: `preview-css-tokens.test.ts` allows its stylesheet only `editorFg`,
  `editorBg`, `syntax*`, `border` and `accent`, and no colour function at all — the SC-005 contrast gate.
  The gate wins over token uniformity; the difference is recorded rather than argued away, and the
  user-visible result (subtle at rest, definite on hover) is the same.
- **Alternatives**: a `linkUnderlineRest` token (rejected — three tokens for two states, and a theme
  could set it to something that contradicts its own hover); opacity on the mark (rejected — it fades the
  text as well, which FR-135 forbids).

## R41. Claiming a range and publishing a link are different questions *(2026-09-20, round five; D7)*

- **Decision**: `scanLinkLine` claims **every** bare email address and publishes only the allowed ones;
  `detectBareEmailSpans` takes no allowlist, and `bareEmailsAreLinks` is asked separately by the caller
  (spec FR-185, FR-186).
- **Rationale**: the allowlist answers "is this a **link**?" It says nothing about "is this a **file**?",
  and a bare address must stop being a path whatever the answer to the first question is. The gated
  version of this detector — the obvious one, written first — passed four of six cases and then handed
  the address back to the path grammar as soon as `mailto` was removed, which is the reported defect by a
  second route. The scan already had this asymmetry for a **refused** scheme; round five names it so the
  next grammar inherits it.
- **Why the scheme is supplied rather than written**: the alternative is a new span kind for addresses,
  with its own class, menu and wording. A bare address *is* an action protocol (FR-157 class 4) and
  behaves like one in every respect; only its spelling differs. Supplying the scheme reuses the class,
  the route, the Link menu, the hover title and the allowlist, and costs one documented asymmetry —
  `uri` is not the span's text (data-model §17.7).
- **Why not fix it in the path grammar** (refuse a candidate containing `@`): rejected. It would make the
  address *nothing* rather than a mail link, contradicting the preview (FR-104, FR-166) and the
  maintainer's ruling; and it would refuse legitimate paths that carry an `@`, which the two control
  cases exist to protect.
- **Alternatives for the grammar itself**: a full RFC 5322 local part (rejected — it admits quoted
  strings containing `/` and `\`, which is precisely what keeps paths out); no word-boundary rule
  (rejected — it matches inside `D:\p\a.b@c.com\x.ts` and inside a scoped package).

## R42. A readout names nothing rather than the wrong thing *(2026-09-20, round five; D8)*

- **Decision** (maintainer, M19, choosing between the two options put to them): a rooted path that is not
  a drive form reads out and titles **as its own text**. `linkFirstReadingByName`'s step 3 stops
  answering the project root (spec FR-187).
- **Rationale**: R6 and FR-024 are both correct — the project root **is** the first reading
  `resolveCandidate` tries — but *tried first* is not *where it goes*. Resolution falls through to
  `IPathForms.fromMountTable` when that reading does not exist, so which one wins is a fact about the
  disk, and a hover resolves nothing (R25, FR-155). The readout was publishing a **candidate** as a
  **destination**.
- **Why it cannot be narrowed to the mount points**: by name `/tmp`, `/etc/hosts` and `/help` are one
  shape, and `/help` really does read against the project root (FR-174). Telling them apart means
  handing the renderer a mount table it has no port to — Principle II, and `links-no-os-names.test.ts`
  forbids the spellings outright — and it would still be a guess about a disk the hover must not touch.
- **The considered alternative: resolve on hover.** Put to the maintainer and **rejected**. It tells the
  whole truth, and it costs a `stat` per hovered link plus a readout that arrives late on a slow share —
  and it re-opens the per-volume-root gate and back-off (FR-121, FR-122a) on the hover path, which is
  the trade round four deliberately settled the other way (M1, FR-155, SC-021). If it is ever
  reconsidered, the cost to weigh is not one `stat`: it is hover-time disk access returning to a design
  that removed it on purpose.
- **Other alternatives**: show both readings ("`<project>\tmp` or the platform's `/tmp`") — rejected, a
  readout is one line and a user reads the first thing in it; show the project-root reading only when a
  project is open — rejected, it makes the wrongness intermittent rather than absent.
- **Worth recording as drift**: `path-by-name.ts`'s module header already stated the as-written rule for
  its undecidable cases while step 3 did the opposite. Where a module documents a rule its code does not
  keep, the documentation is evidence of intent, not decoration — this one was right for a year, and the
  bug was reachable the whole time.
