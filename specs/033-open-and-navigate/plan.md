...
|---|---|---|---|
| Keystroke → ranked, capped list (pure) | **100 ms** at 50,000 paths (SC-002) | unit, T008, synthetic corpus, worst of N | the pipeline is pure, so this is the number that is actually about the algorithm |
| Keystroke → rendered list (in-app) | **250 ms** on a realistic project | E2E, T026 | deliberately looser: it additionally pays for Electron IPC, React render and paint. Stated here so it can be argued with when it fails — a task-local threshold that appears in no artifact is one nobody can challenge |
...
1. **US1 — Quick Open** (P1). The index, the picker's five new props, the chord, the toolbar button.
