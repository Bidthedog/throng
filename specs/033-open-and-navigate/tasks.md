...
- [ ] T071 [US5] Verify the **cog-menu contradiction stays resolved**, and that the resolution is consistent across all three artifacts. It was settled on 2026-08-15, before implementation began: `spec.md` FR-052 and US5's acceptance scenario AS-5 both now state one **Application** section with **no divider**, and `plan.md`'s Complexity Tracking row records it resolved. This task is a check, not an amendment — confirm no artifact still asks for the forbidden divider (grep the spec, plan and contracts for "separated from the diagnostic"), and that `menu-sections.e2e.ts` (T055) asserts the cog menu carries no divider. **The original wording of this task told you to make an amendment that has already landed; doing it twice would re-supersede corrected text.**
