...
**Prerequisites**: [spec.md](./spec.md) (75 FRs, 16 SCs, 6 user stories), [plan.md](./plan.md),
...
- [ ] T001 Capture the pre-change baseline so any later red is attributable to this feature: run `npm run lint`, `npm run typecheck` and `npx vitest run --project unit` from the worktree root per the `running-tests` skill, writing the combined output to `.claude-scratch/033-baseline.txt`. Confirm `packages/ui/tests/unit/shard-plan.test.ts` and `packages/ui/tests/unit/icon-tokens-exist.test.ts` are green before any file is added.
...
- [ ] T026 [US1] RED — create `packages/ui/tests/e2e/quick-open-perf.e2e.ts` covering SC-002's architectural half and FR-015: instrument the preload bridge in-page, type ten characters, and assert **zero** `throng:files:*` and zero `throng:fileIndex:subscribe` calls occur on the keystroke path; measure in-page keystroke-to-list latency against a realistic project in the style of `packages/ui/tests/e2e/editor-highlight-perf.e2e.ts`; and, against a large seeded fixture, assert the modal opened before enumeration finishes renders its "still listing" state and then its results rather than a partial list presented as whole. Run it and record the failure.
...
- [ ] T057 [US5] RED — write `packages/ui/tests/unit/menu-sections.test.ts` invoking **every** builder over a table of fixtures — `buildContextMenuItems` (`explorer/context-menu-items.ts`), `editorContentMenu` (`editor/content-menu.ts`), `panelHeaderMenu` (`workspace/panel-header-menu.ts`), `terminalContentMenu` (`terminal/terminal-content-menu.ts`), and the tab, cog and Key Bindings chord builders — asserting each item declares a section, that sections appear in `MENU_SECTION_ORDER`, and that the derived divider positions are exactly the boundaries. The Red here is an **unresolvable import** of the two not-yet-extracted modules; run `npx vitest run --project unit packages/ui/tests/unit/menu-sections.test.ts` and record that output as the Red.
...
- [ ] T065 [P] [US5] Declare sections in `packages/ui/src/renderer/title-bar/cog-menu.tsx` — all five items (Settings, Key Bindings, Themes, Open Logs Folder, About throng) are **Application**, therefore **one section and no divider**. See T070: this is the recorded contradiction with FR-052.
...
- [ ] T071 [US5] Resolve the **cog-menu contradiction** before US5 is marked done: FR-052 requires the cog menu's three preferences destinations to be separated from Open Logs Folder and About, while FR-047's own table and the constitution place all five in **Application** and FR-050 permits a divider only at a section boundary. Amend `specs/033-open-and-navigate/spec.md` FR-052 to record the superseded text and state the implemented behaviour (one Application section, no divider), following the spec's existing *(Corrected 2026-08-15 …)* convention, and update the Complexity Tracking row in `specs/033-open-and-navigate/plan.md` to say it is resolved.
...
- Two known spec tensions are carried in [plan.md § Complexity Tracking](./plan.md) and must be
  closed before the branch merges: the cog-menu contradiction (T071, a spec amendment) and SC-011's
  literal impossibility (T072, T083 — plus `menu-keyboard.e2e.ts`, which FR-053a itself requires
  changing and which SC-011 does not name).
