...
- [x] **Rich code editors** — syntax highlighting for 31 languages on the editor panels above, detected by extension with a status-strip language picker that remembers the choice per file; a right-click content menu (cut/copy/paste, Select All, Undo/Redo, Set Language…) so the editor is usable with the mouse alone; `cut-line` on Ctrl+X with shape-aware paste (a cut line returns as a line, above the caret); rectangular (column) selection by Alt+drag or `Shift+Alt+Arrow`, with typing, deletion, cut and paste applying per row; per-language tab/space indentation in which the FILE's own style outranks every preference; and one shared undo stack per document — mirrored views of a file undo each other's edits, and the history survives a crash
- [ ] **`.editorconfig` support** — honour a project's `.editorconfig`, cascading over the user's per-language indentation settings
- [ ] **Exact-filename language descriptors** — highlight files matched by whole filename rather than extension (`.gitignore`, `.env`, `Dockerfile`, `Makefile`)
- [ ] **Line-ending conversion** — an explicit "convert to LF / CRLF" action on a document, surfaced from the editor status strip
...
- [x] **Granular reset controls** — the settings/key-binding half of the shipped-defaults restore API, finally reachable from the preferences window: a **per-item reset** on every setting leaf and key-binding action, shown only while that item differs from its shipped value (so the affordance doubles as the "modified" cue) and applied immediately, restoring a binding's **full** shipped chord set; a **per-editor reset** for Settings and Key Bindings; and an atomic **Reset All Preferences** (settings + key bindings + built-in themes) behind a confirmation that states both what is reset **and** what survives — projects, window layout, workspace state and custom themes are never touched. Feature 007's session undo is retained and renamed **Revert All Preferences**. A failed reset surfaces a dismissable "nothing was changed" notice, and the app's second, drifting set of defaults is retired so every reset reads the same record
- [ ] **Unify the confirmation / error / notice surfaces** ([#48](https://github.com/Bidthedog/throng/issues/48)) — the preferences window's inline strip, the themes editor's modal dialog and the main window's dismissable error panels are three ways of saying the same thing; consolidate them into one toast-based notification system
...
- [ ] **Full keyboard-only support** — an app-wide accessibility pass: focusability, keyboard operation, accessible names and roles, and a visible focus indicator for *every* interactive control
- [ ] **Theme accessibility** — bring the `throng`, `Light`, `Snake` and `Claude` themes up to WCAG AA and gate them alongside Bash / SUBNET / Cyberpunk, and mark which themes are accessible in the theme picker (deliberately low-contrast themes stay ungated by design)
